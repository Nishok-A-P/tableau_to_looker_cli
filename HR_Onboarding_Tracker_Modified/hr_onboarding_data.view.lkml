view: hr_onboarding_data {
  ...
  dimension: c_days_taken_to_complete_abs_calc {
    ...
    type: number
    sql: ABS(${TABLE}.days_taken_to_complete) ;;
    hidden: yes
  ...
  dimension: c_diff_overall_currently_onboarding_calc {
    description: "Row-level calculation for c_diff_overall_currently_onboarding: [c: Max Stage Time Spent (copy)_1879127000802996282]-[Calculation_1879127000797835319] /*works out the difference between the avg time taken to complete stage and the avg time current consultants in that stage have spent there */"
    type: number
  ...
  measure: c_max_stage_complete {
    ...
    type: number
    sql: (SELECT MAX(CASE WHEN NOT ${TABLE}.date IS NULL THEN ${TABLE}.stage_order ELSE NULL END) FROM ${TABLE} GROUP BY name) ;;
  ...
  measure: c_max_stage_1 {
    ...
    type: number
    sql: ((SELECT MAX(CASE WHEN NOT ${TABLE}.date IS NULL THEN ${TABLE}.stage_order ELSE NULL END) FROM ${TABLE} GROUP BY name) + 1) ;;
  ...
  dimension: c_max_stage_complete_description {
    ...
    type: string
    sql: (SELECT MAX(CASE WHEN (${c_max_stage_complete} = ${TABLE}.stage_order) THEN ${TABLE}.stage_description ELSE NULL END) FROM ${TABLE} GROUP BY name) ;;
  ...
  dimension: c_currently_onboarding {
    ...
    type: yesno
    sql: (SELECT MAX(CASE WHEN (${TABLE}.date IS NULL AND (${TABLE}.stage_order = 13)) THEN TRUE ELSE FALSE END) FROM ${TABLE} GROUP BY name) ;;
  ...
  dimension: c_candidate_selected {
    ...
    type: string
    sql: (SELECT MAX(CASE WHEN (${TABLE}.stage_order = 1) THEN ${TABLE}.date ELSE NULL END) FROM ${TABLE} GROUP BY name) ;;
  ...
  measure: c_max_stage_comparison_to_avg_per_name {
    ...
    type: number
    sql: (${c_max_stage_time_spent} - (SELECT AVG(CASE WHEN (${c_max_stage_1} = ${TABLE}.stage_order) THEN ${c_avg_days_to_complete_lod_onboarded} ELSE NULL END) FROM ${TABLE} GROUP BY name)) ;;
  ...
  measure: c_avg_days_to_complete_lod_onboarded {
    ...
    type: number
    sql: (SELECT AVG(CASE WHEN NOT ${c_currently_onboarding} THEN ${TABLE}.days_taken_to_complete ELSE NULL END) FROM ${TABLE} GROUP BY stage_order) ;;
  ...
  dimension: c_max_stage_1_owner {
    ...
    type: string
    sql: (SELECT MAX(CASE WHEN (${c_max_stage_1} = ${TABLE}.stage_order) THEN ${TABLE}.owner ELSE NULL END) FROM ${TABLE} GROUP BY name) ;;
  ...
  measure: c_overall_avg_to_complete_stage_max_stage_1 {
    ...
    type: number
    sql: AVG(CASE WHEN (${c_max_stage_1} = ${TABLE}.stage_order) THEN ${c_avg_days_to_complete_lod_onboarded} ELSE NULL END) ;;
  ...
  measure: c_diff_overall_currently_onboarding {
    ...
    type: number
    sql: ${c_diff_overall_currently_onboarding_calc} ;;
    # Original Tableau formula: [c: Max Stage Time Spent (copy)_1879127000802996282]-[Calculation_1879127000797835319] /*works out the difference between the avg time taken to complete stage and the avg time current consultants in that stage have spent there */
  }
  ...
  dimension: c_target_placement_date_in_past {
    ...
    type: yesno
    sql: CASE WHEN (${c_currently_onboarding} AND (${TABLE}.estimated_start_date < CURRENT_DATE())) THEN TRUE ELSE FALSE END ;;
  ...
  dimension: stage_order_string {
    ...
    type: string
    sql: CAST(${TABLE}.stage_order AS STRING) ;;
  ...
  dimension: c_candidate_started {
    ...
    type: string
    sql: (SELECT MAX(CASE WHEN (${TABLE}.stage_order = 13) THEN ${TABLE}.date ELSE NULL END) FROM ${TABLE} GROUP BY name) ;;
  ...
  dimension: c_max_stage_complete_description_1 {
    ...
    type: string
    sql: (SELECT MAX(CASE WHEN (${c_max_stage_1} = ${TABLE}.stage_order) THEN ${c_stage_description_30char} ELSE NULL END) FROM ${TABLE} GROUP BY name) ;;
  ...
  dimension: c_max_stage_complete_description_1_long {
    ...
    type: string
    sql: (SELECT MAX(CASE WHEN (${c_max_stage_1} = ${TABLE}.stage_order) THEN ${TABLE}.stage_description ELSE NULL END) FROM ${TABLE} GROUP BY name) ;;
  ...
  dimension: c_max_stage_complete_order_1 {
    ...
    type: number
    sql: (SELECT MAX(CASE WHEN (${c_max_stage_1} = ${TABLE}.stage_order) THEN ${TABLE}.stage_order ELSE NULL END) FROM ${TABLE} GROUP BY name) ;;
