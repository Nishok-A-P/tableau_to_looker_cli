...
measure: c_max_stage_complete {
    ...
    type: number
    sql: (
        {% if hr_onboarding_data.name._in_query %}
        SELECT MAX(CASE WHEN NOT `Date` IS NULL THEN `Stage Order` ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Name`, MAX(CASE WHEN NOT ods.`Date` IS NULL THEN ods.`Stage Order` ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
            WHERE 1=1
                {% if hr_onboarding_data.stage_description._in_query %}
              AND `Stage Description` = ${stage_description}
            {% endif %}
            {% if hr_onboarding_data.stage_short_name._in_query %}
              AND `Stage Short Name` = ${stage_short_name}
            {% endif %}
            {% if hr_onboarding_data.team._in_query %}
              AND `Team` = ${team}
            {% endif %}
            {% if hr_onboarding_data.owner._in_query %}
              AND `Owner` = ${owner}
            {% endif %}
            {% if hr_onboarding_data.date_raw._in_query %}
              AND `Date` = ${date_raw}
            {% endif %}
            {% if hr_onboarding_data.stage._in_query %}
              AND `Stage` = ${stage}
            {% endif %}
            {% if hr_onboarding_data.estimated_start_date_raw._in_query %}
              AND `Estimated Start Date` = ${estimated_start_date_raw}
            {% endif %}
            ) filtered_dimensions
            ON ods.`Name` = filtered_dimensions.`Name`
            GROUP BY ods.`Name`
        ) aggregated_totals
        {% endif %}
        -- LOD
...
measure: c_max_stage_1 {
  ...
  dimension: c_max_stage_complete_description {
    ...
    sql: (
        ...
        FROM (
            ...
            FROM `None`
            WHERE 1=1
                {% if None.stage_description._in_query %}
              AND `Stage Description` = ${stage_description}
            {% endif %}
            {% if None.stage_short_name._in_query %}
              AND `Stage Short Name` = ${stage_short_name}
            {% endif %}
            {% if None.team._in_query %}
              AND `Team` = ${team}
            {% endif %}
            {% if None.owner._in_query %}
              AND `Owner` = ${owner}
            {% endif %}
            {% if None.date_raw._in_query %}
              AND `Date` = ${date_raw}
            {% endif %}
            {% if None.stage._in_query %}
              AND `Stage` = ${stage}
            {% endif %}
            {% if None.estimated_start_date_raw._in_query %}
              AND `Estimated Start Date` = ${estimated_start_date_raw}
            {% endif %}
            ) filtered_dimensions
            ON ods.`Name` = filtered_dimensions.`Name`
        ...
        {% endif %}
        -- LOD
    ) = `Stage Order`) THEN `Stage Description` ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Name`, MAX(CASE WHEN ((
        {% if None.name._in_query %}
        SELECT MAX(CASE WHEN NOT ods.`Date` IS NULL THEN ods.`Stage Order` ELSE NULL END)
        FROM ods.`None`
        WHERE ods.`Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Nameods.`, MAX(CASE WHEN NOT ods.`Dateods.` IS NULL THEN ods.`Stage Orderods.` ELSE NULL END) AS dimension_totals
            FROM `Noneods.` ods
            JOIN (
            SELECT DISTINCT `Nameods.`
            FROM `Noneods.`
            WHERE 1=1
                {% if None.stage_description._in_query %}
              AND `Stage Descriptionods.` = ${stage_description}
            {% endif %}
            {% if None.stage_short_name._in_query %}
              AND `Stage Short Nameods.` = ${stage_short_name}
            {% endif %}
            {% if None.team._in_query %}
              AND `Teamods.` = ${team}
            {% endif %}
            ...
            {% endif %}
            {% if None.date_raw._in_query %}
              AND `Dateods.` = ${date_raw}
            {% endif %}
            {% if None.stage._in_query %}
              AND `Stageods.` = ${stage}
            {% endif %}
            {% if None.estimated_start_date_raw._in_query %}
              AND `Estimated Start Dateods.` = ${estimated_start_date_raw}
            {% endif %}
            ) filtered_dimensions
            ON ods.`Nameods.` = filtered_dimensions.`Nameods.`
            GROUP BY ods.`Nameods.`
        ) aggregated_totals
        {% endif %}
        -- LOD
    ) = `Stage Orderods.`) THEN `Stage Description` ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
  ...
  dimension: c_currently_onboarding {
    ...
    sql: (
        {% if hr_onboarding_data.name._in_query %}
        SELECT MAX(CASE WHEN (`Date` IS NULL AND (`Stage Order` = 13)) THEN TRUE ELSE FALSE END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Name`, MAX(CASE WHEN (ods.`Date` IS NULL AND (ods.`Stage Order` = 13)) THEN TRUE ELSE FALSE END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
  ...
  dimension: c_candidate_selected {
    ...
    sql: (
        {% if hr_onboarding_data.name._in_query %}
        SELECT MAX(CASE WHEN (`Stage Order` = 1) THEN `Date` ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Name`, MAX(CASE WHEN (ods.`Stage Order` = 1) THEN ods.`Date` ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
...
measure: c_days_taken_to_complete_abs {
  ...
  dimension: c_max_stage_complete_date {
    ...
    sql: (
        {% if hr_onboarding_data.name._in_query %}
        SELECT MAX(CASE WHEN NOT `Date` IS NULL THEN `Date` ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Name`, MAX(CASE WHEN NOT ods.`Date` IS NULL THEN ods.`Date` ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
...
measure: c_max_stage_comparison_to_avg_per_name {
    ...
    sql: (${c_max_stage_time_spent} - (
        ...
        {% if None.stage_order._in_query %}
        SELECT AVG(CASE WHEN NOT (
        {% if None.name._in_query %}
        SELECT MAX(CASE WHEN (`Date` IS NULL AND (`Stage Order` = 13)) THEN TRUE ELSE FALSE END)
        FROM `None`
        WHERE `Name` = ${name}
        ...
        {% endif %}
        -- LOD
    ) THEN `Days Taken To Complete` ELSE NULL END)
        FROM `None`
        WHERE `Stage Order` = ${stage_order}
        {% else %}
        SELECT AVG(dimension_totals)
        FROM (
            SELECT ods.`Stage Order`, AVG(CASE WHEN NOT (
        {% if None.name._in_query %}
        SELECT MAX(CASE WHEN (ods.`Date` IS NULL AND (ods.`Stage Order` = 13)) THEN TRUE ELSE FALSE END)
        FROM ods.`None`
        WHERE ods.`Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Nameods.`, MAX(CASE WHEN (ods.`Dateods.` IS NULL AND (ods.`Stage Orderods.` = 13)) THEN TRUE ELSE FALSE END) AS dimension_totals
            FROM `Noneods.` ods
            JOIN (
            SELECT DISTINCT `Nameods.`
            FROM `Noneods.`
        ...
        {% endif %}
        -- LOD
    ) THEN `Days Taken To Complete` ELSE NULL END) AS dimension_totals
            FROM `None` ods
            JOIN (
            SELECT DISTINCT `Stage Order`
            FROM `None`
            WHERE 1=1
                {% if None.stage_description._in_query %}
              AND `Stage Description` = ${stage_description}
            ...
            ON ods.`Stage Order` = filtered_dimensions.`Stage Order`
            GROUP BY ods.`Stage Order`
        ) aggregated_totals
        {% endif %}
        -- LOD
    ) ELSE NULL END)
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Orderods.`) THEN (
        {% if None.stage_order._in_query %}
        SELECT AVG(CASE WHEN NOT (
        {% if None.name._in_query %}
        SELECT MAX(CASE WHEN (`Dateods.` IS NULL AND (`Stage Orderods.` = 13)) THEN TRUE ELSE FALSE END)
        FROM `Noneods.`
        WHERE `Nameods.` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        ...
        {% endif %}
        -- LOD
    ) THEN `Days Taken To Completeods.` ELSE NULL END)
        FROM `Noneods.`
        WHERE `Stage Orderods.` = ${stage_order}
        {% else %}
        SELECT AVG(dimension_totals)
        FROM (
            SELECT ods.`Stage Orderods.`, AVG(CASE WHEN NOT (
        {% if None.name._in_query %}
        SELECT MAX(CASE WHEN (ods.`Dateods.` IS NULL AND (ods.`Stage Orderods.` = 13)) THEN TRUE ELSE FALSE END)
        FROM ods.`Noneods.`
        WHERE ods.`Nameods.` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Nameods.`, MAX(CASE WHEN (ods.`Dateods.` IS NULL AND (ods.`Stage Orderods.` = 13)) THEN TRUE ELSE FALSE END) AS dimension_totals
            FROM ods.`Noneods.` ods
            JOIN (
            SELECT DISTINCT ods.`Nameods.`
            FROM ods.`Noneods.`
            WHERE 1=1
                {% if None.stage_description._in_query %}
              AND ods.`Stage Descriptionods.` = ${stage_description}
            {% endif %}
            {% if None.stage_short_name._in_query %}
              AND ods.`Stage Short Nameods.` = ${stage_short_name}
            {% endif %}
            {% if None.team._in_query %}
              AND ods.`Teamods.` = ${team}
            {% endif %}
            {% if None.owner._in_query %}
              AND ods.`Ownerods.` = ${owner}
            {% endif %}
            {% if None.date_raw._in_query %}
              AND ods.`Dateods.` = ${date_raw}
            {% endif %}
            {% if None.stage._in_query %}
              AND ods.`Stageods.` = ${stage}
            {% endif %}
            {% if None.estimated_start_date_raw._in_query %}
              AND ods.`Estimated Start Dateods.` = ${estimated_start_date_raw}
            {% endif %}
            ) filtered_dimensions
            ON ods.`Nameods.` = filtered_dimensions.`Nameods.`
            GROUP BY ods.`Nameods.`
        ...
        {% endif %}
        -- LOD
    ) THEN ods.`Days Taken To Complete` ELSE NULL END) AS dimension_totals
            FROM ods.`None` ods
            JOIN (
            SELECT DISTINCT ods.`Stage Order`
            FROM ods.`None`
            WHERE 1=1
                {% if None.stage_description._in_query %}
              AND ods.`Stage Description` = ${stage_description}
            {% endif %}
            {% if None.stage_short_name._in_query %}
              AND ods.`Stage Short Name` = ${stage_short_name}
            {% endif %}
            {% if None.team._in_query %}
              AND ods.`Team` = ${team}
            {% endif %}
            ...
            {% endif %}
            {% if None.date_raw._in_query %}
              AND ods.`Date` = ${date_raw}
            {% endif %}
            {% if None.stage._in_query %}
              AND ods.`Stage` = ${stage}
            {% endif %}
            {% if None.estimated_start_date_raw._in_query %}
              AND ods.`Estimated Start Date` = ${estimated_start_date_raw}
            {% endif %}
            ) filtered_dimensions
            ON ods.`Stage Orderods.` = filtered_dimensions.`Stage Orderods.`
            GROUP BY ods.`Stage Order`
        ) aggregated_totals
        {% endif %}
        -- LOD
    ) ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
...
measure: c_avg_days_to_complete_lod_onboarded {
    ...
    type: number
    sql: (
        {% if hr_onboarding_data.stage_order._in_query %}
        SELECT AVG(CASE WHEN NOT (
        {% if None.name._in_query %}
        SELECT MAX(CASE WHEN (`Date` IS NULL AND (`Stage Order` = 13)) THEN TRUE ELSE FALSE END)
        FROM `None`
  ...
  dimension: c_max_stage_1_owner {
    ...
    sql: (
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Order`) THEN `Owner` ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Orderods.`) THEN `Owner` ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
...
measure: c_diff_overall_currently_onboarding_flag {
  ...
  dimension: c_candidate_started {
    ...
    sql: (
        {% if hr_onboarding_data.name._in_query %}
        SELECT MAX(CASE WHEN (`Stage Order` = 13) THEN `Date` ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.`Name`, MAX(CASE WHEN (ods.`Stage Order` = 13) THEN ods.`Date` ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
...
measure: c_max_stage_time_spent_dummy {
  ...
  dimension: c_max_stage_complete_description_1 {
    ...
    sql: (
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Order`) THEN CASE WHEN (LENGTH(`Stage Description`) > 30) THEN (LEFT(`Stage Description`, 30) || '...') ELSE `Stage Description` END ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Orderods.`) THEN CASE WHEN (LENGTH(`Stage Descriptionods.`) > 30) THEN (LEFT(`Stage Descriptionods.`, 30) || '...') ELSE `Stage Description` END ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
  ...
  dimension: c_max_stage_complete_description_1_long {
    ...
    sql: (
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Order`) THEN `Stage Description` ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Orderods.`) THEN `Stage Description` ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
  ...
  dimension: c_max_stage_complete_order_1 {
    ...
    sql: (
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Order`) THEN `Stage Order` ELSE NULL END)
        FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
        WHERE `Name` = ${name}
        {% else %}
        SELECT MAX(dimension_totals)
        ...
        {% endif %}
        -- LOD
    ) + 1) = `Stage Orderods.`) THEN `Stage Order` ELSE NULL END) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data` ods
            JOIN (
            SELECT DISTINCT `Name`
            FROM `tableau-to-looker-migration.Super_Store_Sales.hr_onboarding_data`
  ...
  dimension: stage_description1_attribute_derived {
    ...
    type: string
    sql: CASE WHEN MIN(${TABLE}.`Stage Description`) IS NULL THEN NULL WHEN MIN(${TABLE}.`Stage Description`) = MAX(${TABLE}.`Stage Description`) THEN CAST(MIN(${TABLE}.`Stage Description`)as STRING) ELSE '*' END ;;
...
measure: days_taken_to_complete_avg_derived {
...
measure: stage_order1_min_derived {
  ...
  dimension: c_max_stage_complete_description_1_long_attribute_derived {
    ...
    type: string
    sql: CASE WHEN MIN(${c_max_stage_complete_description_1_long}) IS NULL THEN NULL WHEN MIN(${c_max_stage_complete_description_1_long}) = MAX(${c_max_stage_complete_description_1_long}) THEN CAST(MIN(${c_max_stage_complete_description_1_long})as STRING) ELSE '*' END ;;
...
measure: c_days_taken_to_complete_abs_avg_derived {
  ...
  dimension: c_stage_description_30char_attribute_derived {
    ...
    type: string
    sql: CASE WHEN MIN(${c_stage_description_30char}) IS NULL THEN NULL WHEN MIN(${c_stage_description_30char}) = MAX(${c_stage_description_30char}) THEN CAST(MIN(${c_stage_description_30char})as STRING) ELSE '*' END ;;
  ...
  dimension: date1_attribute_derived {
    ...
    type: date
    sql: CASE WHEN MIN(${TABLE}.`Date`) IS NULL THEN NULL WHEN MIN(${TABLE}.`Date`) = MAX(${TABLE}.`Date`) THEN CAST(MIN(${TABLE}.`Date`)as STRING) ELSE '*' END ;;
...
measure: days_taken_to_complete_sum_derived {
