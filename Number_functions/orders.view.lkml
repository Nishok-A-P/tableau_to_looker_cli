view: orders {
  ...
  dimension: div_calc {
    description: "Row-level calculation for div: DIV(INT([Sales]),INT([Discount]))"
    type: number
    sql: DIV(CAST(${TABLE}.sales AS INT64), CAST(${TABLE}.discount AS INT64)) ;;
    hidden: yes
    # Original Tableau formula: DIV(INT([Sales]),INT([Discount]))
  }
  ...
  measure: div {
    description: "Calculated field: DIV(INT([Sales]),INT([Discount]))"
    type: number
    sql: ${div_calc} ;;
    # Original Tableau formula: DIV(INT([Sales]),INT([Discount]))
  }
