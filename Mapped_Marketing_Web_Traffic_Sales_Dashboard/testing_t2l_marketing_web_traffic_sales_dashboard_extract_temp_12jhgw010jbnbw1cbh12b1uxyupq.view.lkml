view: testing_t2l_marketing_web_traffic_sales_dashboard_extract_temp_12jhgw010jbnbw1cbh12b1uxyupq {
  ...
  dimension: 15_08_2020_calc {
    ...
    sql: 0 ;;
    hidden: yes
    # TODO: Manual conversion required - complex LOD expression ORIGINAL_FORMULA: IF ISDATE("2020-08-15") = True THEN [Sale Amount]/5 ELSE [Sale Amount] END REASON: Complex LOD function contains IF/CASE/nested conditions and Functions
  }
...
measure: sales_amount_percent_chg_vs_py {
    ...
    # TODO: Manual conversion required - complex LOD expression ORIGINAL_FORMULA: ( SUM([Calculation_1799188053049602048])-SUM([Sales CY (copy)_1799188053049700353]) ) / SUM([Sales CY (copy)_1799188053049700353]) REASON: Complex LOD function contains IF/CASE/nested conditions and Functions
  }
...
measure: visitors_by_lead_source_py {
    ...
    # TODO: Manual conversion required - complex LOD expression ORIGINAL_FORMULA: COUNT(if DATETRUNC('year', [Date]) = [Calculation_1799188053689634893] THEN [Customer ID] END) REASON: Complex LOD function contains IF/CASE/nested conditions and Functions
  }
...
measure: customer_count_per_day {
    ...
    type: number
    sql: (
        {% if testing_t2l_marketing_web_traffic_sales_dashboard_extract_temp_12jhgw010jbnbw1cbh12b1uxyupq.date_raw._in_query %}
        SELECT COUNT(`Customer ID`)
        FROM `tableau-to-looker-migration.Tableau_To_BigQuery.Testing_T2L_Marketing_Web_Traffic_Sales_Dashboard_Social_Media_c_TEMP_1p3v7ee0ranfb01ggj6sj07ic7hi`
        WHERE `Date` = ${date_raw}
        {% else %}
        SELECT COUNT(dimension_totals)
        FROM (
            SELECT ods.`Date`, COUNT(ods.`Customer ID`) AS dimension_totals
            FROM `tableau-to-looker-migration.Tableau_To_BigQuery.Testing_T2L_Marketing_Web_Traffic_Sales_Dashboard_Social_Media_c_TEMP_1p3v7ee0ranfb01ggj6sj07ic7hi` ods
            JOIN (
            SELECT DISTINCT `Date`
            FROM `tableau-to-looker-migration.Tableau_To_BigQuery.Testing_T2L_Marketing_Web_Traffic_Sales_Dashboard_Social_Media_c_TEMP_1p3v7ee0ranfb01ggj6sj07ic7hi`
            WHERE 1=1
                {% if testing_t2l_marketing_web_traffic_sales_dashboard_extract_temp_12jhgw010jbnbw1cbh12b1uxyupq.converted._in_query %}
              AND `Converted` = ${converted}
            {% endif %}
            {% if testing_t2l_marketing_web_traffic_sales_dashboard_extract_temp_12jhgw010jbnbw1cbh12b1uxyupq.social_media_c._in_query %}
              AND `Social_Media__c` = ${social_media_c}
            {% endif %}
            {% if testing_t2l_marketing_web_traffic_sales_dashboard_extract_temp_12jhgw010jbnbw1cbh12b1uxyupq.date_c_raw._in_query %}
              AND `Date__c` = ${date_c_raw}
            {% endif %}
            {% if testing_t2l_marketing_web_traffic_sales_dashboard_extract_temp_12jhgw010jbnbw1cbh12b1uxyupq.source._in_query %}
              AND `Source` = ${source}
            {% endif %}
            ) filtered_dimensions
            ON ods.`Date` = filtered_dimensions.`Date`
            GROUP BY ods.`Date`
        ) aggregated_totals
        {% endif %}
        -- LOD
...
measure: conversion_rate_percent_chg_vs_py {
    ...
    # TODO: Manual conversion required - complex LOD expression ORIGINAL_FORMULA: ( [Calculation_621496750485102625]-[Calculation_621496750490652711] ) / [Calculation_621496750490652711] REASON: Complex LOD function contains IF/CASE/nested conditions and Functions
  }
...
measure: customer_id_cy_city_bar_chart {
    ...
    # TODO: Manual conversion required - complex LOD expression ORIGINAL_FORMULA: COUNT(if DATETRUNC('year', [Date]) = [Calculation_1799188053708918879] THEN [Customer ID] END) REASON: Complex LOD function contains IF/CASE/nested conditions and Functions
  }
...
measure: visitors_by_lead_source_cy {
  ...
  dimension: date_attribute_derived {
    ...
    # TODO: Manual conversion required - complex LOD expression ORIGINAL_FORMULA: ATTR([Date]) REASON: Complex LOD function contains IF/CASE/nested conditions and Functions
  }
