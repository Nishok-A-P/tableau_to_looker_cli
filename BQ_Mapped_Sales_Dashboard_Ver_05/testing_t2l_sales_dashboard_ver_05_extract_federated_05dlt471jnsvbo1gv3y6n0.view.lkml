...
measure: customers_lm {
    ...
    type: number
    sql: COUNT(DISTINCT CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${TABLE}.`CustomerID` ELSE NULL END) ;;


    # Original Tableau formula: COUNTD(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [CustomerID] END)
  }

measure: discount_lm {
    description: "Calculated field: AVG(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [Discount] END)"
    type: number
    sql: AVG(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${TABLE}.`Discount` ELSE NULL END) ;;


    # Original Tableau formula: AVG(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [Discount] END)
  }

measure: customers {
    description: "Calculated field: IF [# Customers PM (copy)_1257630217801580569]>=0 THEN \"▲\" END"
    type: number
    sql: CASE WHEN (${customers_diff} >= 0) THEN '▲' ELSE NULL END ;;


    # Original Tableau formula: IF [# Customers PM (copy)_1257630217801580569]>=0 THEN "▲" END
  }
...
measure: customers_diff_percent {
    ...
    type: number
    sql: ((${customers_lm} - ${customers_pm}) / NULLIF(${customers_lm}, 0)) ;;


    # Original Tableau formula: ([# Customers (copy)_1257630217798270977]-[# Customers LM (copy)_1257630217800396818])/[# Customers (copy)_1257630217798270977]
  }
...
measure: customers_pm {
    ...
    type: number
    sql: COUNT(DISTINCT CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${TABLE}.`CustomerID` ELSE NULL END) ;;


    # Original Tableau formula: COUNTD(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [CustomerID] END)
  }
...
measure: orders_lm {
    ...
    type: number
    sql: COUNT(DISTINCT CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${TABLE}.`OrderID` ELSE NULL END) ;;


    # Original Tableau formula: COUNTD(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [OrderID] END)
  }
...
measure: orders {
    ...
    type: number
    sql: CASE WHEN (${orders_diff} < 0) THEN '▼' ELSE NULL END ;;


    # Original Tableau formula: IF [# Orders LM (copy)_1257630217802063899]<0 THEN "▼" END
  }
...
measure: orders_diff_percent {
    ...
    type: number
    sql: ((${orders_lm} - ${orders_pm}) / NULLIF(${orders_lm}, 0)) ;;


    # Original Tableau formula: ([# Orders (copy)_1257630217798397954]-[# Orders LM (copy)_1257630217800462355])/[# Orders (copy)_1257630217798397954]
  }

measure: orders_pm {
    description: "Calculated field: COUNTD(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [OrderID] END)"
    type: number
    sql: COUNT(DISTINCT CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${TABLE}.`OrderID` ELSE NULL END) ;;


    # Original Tableau formula: COUNTD(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [OrderID] END)
  }
...
measure: products_lm {
    ...
    type: number
    sql: COUNT(DISTINCT CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${TABLE}.`ProductID` ELSE NULL END) ;;


    # Original Tableau formula: COUNTD(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [ProductID] END)
  }

measure: products_pm {
    description: "Calculated field: COUNTD(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [ProductID] END)"
    type: number
    sql: COUNT(DISTINCT CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${TABLE}.`ProductID` ELSE NULL END) ;;


    # Original Tableau formula: COUNTD(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [ProductID] END)
  }
...
measure: shipping_cost_unit {
    ...
    type: number
    sql: (SUM(${TABLE}.`ShippingCost`) / NULLIF(SUM(${quantity}), 0)) ;;
...
measure: discount_unit_lm {
    ...
    type: number
    sql: (${discount_lm} / NULLIF(${quantity_lm}, 0)) ;;


    # Original Tableau formula: [# Customers (copy)_1257630217798631428]/[Discount LM (copy)_1257630217799098374]
  }

measure: quantity_lm {
    description: "Calculated field: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [Quantity] END)"
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${quantity} ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [Quantity] END)
  }

measure: discount_pm {
    description: "Calculated field: AVG(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [Discount] END)"
    type: number
    sql: AVG(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${TABLE}.`Discount` ELSE NULL END) ;;


    # Original Tableau formula: AVG(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [Discount] END)
  }

measure: price_lm {
    description: "Calculated field: AVG(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [Price] END)"
    type: number
    sql: AVG(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${TABLE}.`Price` ELSE NULL END) ;;


    # Original Tableau formula: AVG(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [Price] END)
  }
...
measure: price_pm {
    ...
    type: number
    sql: AVG(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${TABLE}.`Price` ELSE NULL END) ;;


    # Original Tableau formula: AVG(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [Price] END)
  }

measure: profit {
    description: "Calculated field: IF [Profit LM (copy)_1257630217802403873]>=0 THEN \"▲\" END"
    type: number
    sql: CASE WHEN (${profit_diff} >= 0) THEN '▲' ELSE NULL END ;;


    # Original Tableau formula: IF [Profit LM (copy)_1257630217802403873]>=0 THEN "▲" END
  }
...
measure: profit_diff_percent {
    ...
    type: number
    sql: ((${profit_lm} - ${profit_pm}) / NULLIF(${profit_lm}, 0)) ;;


    # Original Tableau formula: ([Quantity LM (copy)_1257630217800966165]-[Profit LM (copy)_1257630217801121814])/[Quantity LM (copy)_1257630217800966165]
  }

measure: profit_pm {
    description: "Calculated field: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [Profit] END)"
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${profit} ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [Profit] END)
  }
...
measure: discount_unit {
    ...
    type: number
    sql: (SUM(${TABLE}.`Discount`) / NULLIF(SUM(${quantity}), 0)) ;;
...
measure: quantity {
    ...
    type: number
    sql: CASE WHEN (${quantity_diff} < 0) THEN '▼' ELSE NULL END ;;


    # Original Tableau formula: IF [Quantity LM (copy)_1257630217802543141]<0 THEN "▼" END
  }
...
measure: quantity_diff_percent {
    ...
    type: number
    sql: ((${quantity_lm} - ${quantity_pm}) / NULLIF(${quantity_lm}, 0)) ;;


    # Original Tableau formula: ([Discount LM (copy)_1257630217799098374]-[Quantity LM (copy)_1257630217800065037])/[Discount LM (copy)_1257630217799098374]
  }
...
measure: shipping_cost_lm {
    ...
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${TABLE}.`ShippingCost` ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [ShippingCost] END)
  }
...
measure: total_sales_lm {
    ...
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${TABLE}.`TotalSales` ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [TotalSales] END)
  }

measure: quantity_pm {
    description: "Calculated field: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [Quantity] END)"
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${quantity} ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [Quantity] END)
  }
...
measure: returned_quantity_lm {
    ...
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${TABLE}.`ReturnedQuantity` ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [ReturnedQuantity] END)
  }

measure: profit_lm {
    description: "Calculated field: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [Profit] END)"
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = ${max_date}) THEN ${profit} ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856] THEN [Profit] END)
  }
...
measure: returned_quantity_pm {
    ...
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${TABLE}.`ReturnedQuantity` ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [ReturnedQuantity] END)
  }
...
measure: shipping_cost_unit_lm {
    ...
    type: number
    sql: (${shipping_cost_lm} / NULLIF(${quantity_lm}, 0)) ;;


    # Original Tableau formula: [Quantity LM (copy)_1257630217799401479]/[Discount LM (copy)_1257630217799098374]
  }
...
measure: shipping_cost_pm {
    ...
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${TABLE}.`ShippingCost` ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [ShippingCost] END)
  }

measure: profit_unit {
    description: "Calculated field: SUM([Profit]) / SUM([Quantity])"
    type: number
    sql: (SUM(${profit}) / NULLIF(SUM(${quantity}), 0)) ;;
...
measure: total_sales {
    ...
    type: number
    sql: CASE WHEN (${total_sales_diff} < 0) THEN '▼' ELSE NULL END ;;


    # Original Tableau formula: IF [Total Sales LM (copy)_1257630217802727465]<0 THEN "▼" END
  }
...
measure: total_sales_diff_percent {
    ...
    type: number
    sql: ((${total_sales_lm} - ${total_sales_pm}) / NULLIF(${total_sales_lm}, 0)) ;;


    # Original Tableau formula: ([Quantity LM (copy)_1257630217799512072]-[Total Sales LM (copy)_1257630217799680009])/[Quantity LM (copy)_1257630217799512072]
  }
...
measure: total_sales_pm {
    ...
    type: number
    sql: SUM(CASE WHEN (EXTRACT(MONTH FROM ${TABLE}.`OrderDate`) = (${max_date} - 1)) THEN ${TABLE}.`TotalSales` ELSE NULL END) ;;


    # Original Tableau formula: SUM(IF MONTH([OrderDate])=[Calculation_1257630217796857856]-1 THEN [TotalSales] END)
  }
...
measure: quantity_sum_derived {
  ...
  dimension: review_stars_attribute_derived {
    ...
    type: string
    sql: (SELECT CASE WHEN MIN(${review_stars}) IS NULL THEN NULL WHEN MIN(${review_stars}) = MAX(${review_stars}) THEN MIN(${review_stars}) ELSE '*' END FROM `tableau-to-looker-migration.Tableau_To_BigQuery.Testing_T2L_Sales_Dashboard_Ver_05_Extract_federated_05dlt471jnsvbo1gv3y6n0` as Testing_T2L_Sales_Dashboard_Ver_05_Extract_federated_05dlt471jnsvbo1gv3y6n0) ;;
