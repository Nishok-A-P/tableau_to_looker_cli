...
measure: inefficient_block_percent_color {
  ...
  dimension: single_facility_user {
    ...
    type: yesno
    sql: (( SELECT COUNTD(facility_code) FROM `tableau-to-looker-migration.Super_Store_Sales.aspen_landing_page` as aspen_landing_page -- LOD ) = 1) ;;
...
measure: inefficient_block_percent {
  ...
  dimension: hide_run_rate_calc {
    ...
    type: yesno
    sql: ({% parameter parameter_7 %} OR ((
        {% if aspen_landing_page.my_date._in_query %}
        SELECT MAX(runrate_current)
        FROM `tableau-to-looker-migration.Super_Store_Sales.aspen_landing_page` as aspen_landing_page
        WHERE my_date = ${my_date}
        {% else %}
        SELECT MAX(dimension_totals)
        FROM (
            SELECT ods.my_date, MAX(ods.runrate_current) AS dimension_totals
            FROM `tableau-to-looker-migration.Super_Store_Sales.aspen_landing_page` ods
            JOIN (
            SELECT DISTINCT my_date
            FROM `tableau-to-looker-migration.Super_Store_Sales.aspen_landing_page as aspen_landing_page`
            WHERE 1=1
                {% if aspen_landing_page.assocproduction_num._in_query %}
              AND assocproduction_num = ${assocproduction_num}
            {% endif %}
            {% if aspen_landing_page.creditapp_num._in_query %}
              AND creditapp_num = ${creditapp_num}
            {% endif %}
            {% if aspen_landing_page.division_vp._in_query %}
              AND division_vp = ${division_vp}
            {% endif %}
            {% if aspen_landing_page.hygproduction_num._in_query %}
              AND hygproduction_num = ${hygproduction_num}
            {% endif %}
            {% if aspen_landing_page.mcdproduction_num._in_query %}
              AND mcdproduction_num = ${mcdproduction_num}
            {% endif %}
            {% if aspen_landing_page.pop_owner._in_query %}
              AND pop_owner = ${pop_owner}
            {% endif %}
            {% if aspen_landing_page.region_description._in_query %}
              AND region_description = ${region_description}
            {% endif %}
            {% if aspen_landing_page.sr_division_vp._in_query %}
              AND sr_division_vp = ${sr_division_vp}
            {% endif %}
            {% if aspen_landing_page.territory._in_query %}
              AND territory = ${territory}
            {% endif %}
            ) filtered_dimensions
            ON ods.my_date = filtered_dimensions.my_date
            GROUP BY ods.my_date
        ) aggregated_totals
        {% endif %}
        -- LOD
    ) = 0)) ;;
...
measure: expected_percent_of_schedule {
  ...
  dimension: max_lastbusinessday {
    ...
    type: string
    sql: ( SELECT MAX(lastbusinessday) FROM `tableau-to-looker-migration.Super_Store_Sales.aspen_landing_page` as aspen_landing_page -- LOD ) ;;
