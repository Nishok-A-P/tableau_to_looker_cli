...
measure: customer_count_per_day {
    ...
    sql: (
        ...
        FROM `tableau-to-looker-migration.Tableau_To_BigQuery.Testing_T2L_Marketing_Web_Traffic_Sales_Dashboard_Extract_TEMP_12jhgw010jbnbw1cbh12b1uxyupq`
        WHERE `Date` = ${date_raw}
        {% else %}
        SELECT COUNT(dimension_totals)
        FROM (
            SELECT ods.`Date`, COUNT(ods.`Customer ID`) AS dimension_totals
            FROM `tableau-to-looker-migration.Tableau_To_BigQuery.Testing_T2L_Marketing_Web_Traffic_Sales_Dashboard_Extract_TEMP_12jhgw010jbnbw1cbh12b1uxyupq` ods
            JOIN (
