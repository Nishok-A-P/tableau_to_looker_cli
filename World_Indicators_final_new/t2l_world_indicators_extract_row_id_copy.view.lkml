...
measure: c_gauge1 {
    ...
    # TODO: Manual conversion required - complex LOD expression ORIGINAL_FORMULA: case [Parameters].[p_trend1 (copy)_-4469259676978860022] when '% Internet Users' then abs(if attr([model]) = 'unfilled' then 1 else 0 end-avg([Internet Usage])) WHEN 'Birth Rate' THEN abs(if attr([model]) = 'unfilled' then 1 else 0 end-avg([Birth Rate])) when 'Population 65+' then abs(if attr([model]) = 'unfilled' then 1 else 0 end-avg([Population 65+])) when 'Health Exp % GDP' then abs(if attr([model]) = 'unfilled' then 1 else 0 end-avg([Health Exp Pct GDP])) when 'Lending Interest' then abs(if attr([model]) = 'unfilled' then 1 else 0 end-avg([Lending Interest])) when 'Business Tax Rate' then abs(if attr([model]) = 'unfilled' then 1 else 0 end-avg([Business Tax Rate])) end REASON: Complex LOD function contains IF/CASE/nested conditions and Functions
  }
...
measure: c_cnt {
    ...
    # TODO: Manual conversion required - complex LOD expression ORIGINAL_FORMULA: {fixed [Region], [Country_Region], [Year] : sum(if [model] = 'filled' then [GDP] end) } REASON: Complex LOD function contains IF/CASE/nested conditions and Functions
  }
