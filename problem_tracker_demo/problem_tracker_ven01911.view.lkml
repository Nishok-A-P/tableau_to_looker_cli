view: problem_tracker_ven01911 {
  ...
  dimension: show_group_calc {
    ...
    type: number
    sql: CASE WHEN (COUNT(DISTINCT ${TABLE}.`Name`) = 1) THEN CASE WHEN MIN(${TABLE}.`Name`) IS NULL THEN NULL WHEN MIN(${TABLE}.`Name`) = MAX(${TABLE}.`Name`) THEN MIN(${TABLE}.`Name`) ELSE '*' END ELSE 'Showing All Groups' END ;;
    hidden: yes
...
measure: none_countd_derived_2 {
  ...
  dimension: none_attribute_derived {
    ...
    type: string
    sql: CASE WHEN MIN(${TABLE}.`Description`) IS NULL THEN NULL WHEN MIN(${TABLE}.`Description`) = MAX(${TABLE}.`Description`) THEN MIN(${TABLE}.`Description`) ELSE '*' END ;;
...
measure: none_day_trunc_derived_2 {
  ...
  dimension: none_attribute_derived_2 {
    ...
    type: string
    sql: CASE WHEN MIN(${TABLE}.`Opened__Incident_`) IS NULL THEN NULL WHEN MIN(${TABLE}.`Opened__Incident_`) = MAX(${TABLE}.`Opened__Incident_`) THEN MIN(${TABLE}.`Opened__Incident_`) ELSE '*' END ;;
