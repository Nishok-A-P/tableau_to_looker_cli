view: problem_tracker_ven01911 {
  ...
  dimension: show_group_calc {
    ...
    type: number
    sql: CASE WHEN (COUNT(DISTINCT ${TABLE}.`Name`) = 1) THEN ANY_VALUE(${TABLE}.`Name`) ELSE 'Showing All Groups' END ;;
    hidden: yes
...
measure: none_countd_derived_2 {
  ...
  dimension: none_attribute_derived {
    ...
    type: string
    sql: ANY_VALUE(${TABLE}.`Description`) ;;
...
measure: none_day_trunc_derived_2 {
  ...
  dimension: none_attribute_derived_2 {
    ...
    type: string
    sql: ANY_VALUE(${TABLE}.`Opened__Incident_`) ;;
