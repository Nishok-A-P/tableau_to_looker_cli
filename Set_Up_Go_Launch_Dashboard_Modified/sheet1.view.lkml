...
measure: mva {
    ...
    type: number
    sql: (SUM(${mva_num}) / NULLIF(SUM(${mva_den}), 0)) ;;
...
measure: selected_month_revenue_copy {
    ...
    type: number
    sql: ((SUM(${selected_month_revenue}) * ${month_length}) / NULLIF(${days_in_current_month}, 0)) ;;
...
measure: non_suag_return_rate {
    ...
    type: number
    sql: (${non_suag_returns} / NULLIF(${non_suag_sales}, 0)) ;;
...
measure: max_date {
  ...
  dimension: market_attribute_derived {
...
measure: su_g_single_sum_derived {
  ...
  dimension: rptmth_month_trunc_derived {
...
measure: pymnt_dt_min_derived {
    description: "Calculated field: MIN([PYMNT_DT])"
    type: number
    sql: MIN(${TABLE}.`PYMNT_DT`) ;;


    # Original Tableau formula: MIN([PYMNT_DT])
  }

  dimension: outlet_attribute_derived {
  ...
  dimension: rptmth_month_derived {
