view: sheet1 {
  ...
  dimension: rownum_calc {
    description: "Row-level calculation for rownum: index()"
    type: number
...
measure: issuecounttot {
  ...
  dimension: scheduledesc_attribute_derived {
    description: "Calculated field: ATTR([Calculation_1071012336442302472])"
    type: string
    sql: CASE WHEN MIN(${scheduledesc}) IS NULL THEN NULL WHEN MIN(${scheduledesc}) = MAX(${scheduledesc}) THEN MIN(${scheduledesc}) ELSE '*' END ;;
    # Original Tableau formula: ATTR([Calculation_1071012336442302472])
  }

  dimension: issue_desc_attribute_derived {
    description: "Calculated field: ATTR([ISSUE_DESC])"
    type: string
    sql: CASE WHEN MIN(${TABLE}.`ISSUE_DESC`) IS NULL THEN NULL WHEN MIN(${TABLE}.`ISSUE_DESC`) = MAX(${TABLE}.`ISSUE_DESC`) THEN MIN(${TABLE}.`ISSUE_DESC`) ELSE '*' END ;;
    # Original Tableau formula: ATTR([ISSUE_DESC])
  }

measure: fixed_max_date_max_derived {
    description: "Calculated field: MAX([Calculation_1532349814560436225])"
    type: number
    sql: MAX(${TABLE}.`Calculation_1532349814560436225`) ;;


    # Original Tableau formula: MAX([Calculation_1532349814560436225])
  }

measure: tcdvol_tlktm_len_max_derived {
    description: "Calculated field: MAX([TCDVOL_TLKTM_LEN])"
    type: number
    sql: MAX(${TABLE}.`TCDVOL_TLKTM_LEN`) ;;


    # Original Tableau formula: MAX([TCDVOL_TLKTM_LEN])
  }

measure: fixed_min_date_min_derived {
    description: "Calculated field: MIN([Calculation_1532349814560366592])"
    type: number
    sql: MIN(${TABLE}.`Calculation_1532349814560366592`) ;;


    # Original Tableau formula: MIN([Calculation_1532349814560366592])
  }

measure: gantt_length_sum_derived {
    description: "Calculated field: SUM([Calculation_1071012336339034112])"
    type: number
    sql: SUM(${gantt_length}) ;;


    # Original Tableau formula: SUM([Calculation_1071012336339034112])
  }

measure: issuecount_sum_derived {
    description: "Calculated field: SUM([Calculation_1071012336348946434])"
    type: number
    sql: SUM(${issuecount}) ;;


    # Original Tableau formula: SUM([Calculation_1071012336348946434])
  }

measure: issuecounttot_sum_derived {
    description: "Calculated field: SUM([IssueCount (copy)_1071012336394305543])"
    type: number
    sql: SUM(${issuecounttot}) ;;


    # Original Tableau formula: SUM([IssueCount (copy)_1071012336394305543])
  }

measure: issuecounttot_sum_derived_2 {
    description: "Calculated field: SUM([IssueCount (copy)_1071012336394305543])"
    type: number
    sql: SUM(${issuecounttot}) ;;
...
measure: issuecount_sum_derived_2 {
    description: "Calculated field: SUM([Calculation_1071012336348946434])"
    type: number
    sql: SUM(${issuecount}) ;;
