...
measure: 0 {
  ...
  dimension: color {
    description: "Calculated field: str(MONTH([Order_Date]))+\"-\"+str(YEAR([Order_Date]))"
    type: string
    sql: ((CAST(EXTRACT(MONTH FROM ${TABLE}.`Order_Date`) AS STRING) || '-') || CAST(EXTRACT(YEAR FROM ${TABLE}.`Order_Date`) AS STRING)) ;;
    # Original Tableau formula: str(MONTH([Order_Date]))+"-"+str(YEAR([Order_Date]))
  }
