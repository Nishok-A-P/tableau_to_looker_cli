view: sheet1 {
  ...
  dimension: none_attribute_derived_2_calc {
    description: "Row-level calculation for none_attribute_derived_2: ATTR([METRIC_RESULT_STR])"
    type: number
    sql: CASE WHEN MIN(${TABLE}.`METRIC_RESULT_STR`) IS NULL THEN NULL WHEN MIN(${TABLE}.`METRIC_RESULT_STR`) = MAX(${TABLE}.`METRIC_RESULT_STR`) THEN MIN(${TABLE}.`METRIC_RESULT_STR`) ELSE '*' END ;;
    hidden: yes
...
measure: box_close_performance {
    ...
    type: number
    sql: CASE WHEN (${boxes_close_percent} < ${box_close_percent_threshold}) THEN 'Below Threshold' ELSE CASE WHEN (${boxes_close_percent} >= ${box_close_percent_threshold}) THEN 'Above Threshold' ELSE NULL END END ;;
...
measure: rprt_date_max_derived {
  ...
  dimension: none_attribute_derived {
    ...
    type: string
    sql: CASE WHEN MIN(${TABLE}.`METRIC_DESC`) IS NULL THEN NULL WHEN MIN(${TABLE}.`METRIC_DESC`) = MAX(${TABLE}.`METRIC_DESC`) THEN MIN(${TABLE}.`METRIC_DESC`) ELSE '*' END ;;
    # Original Tableau formula: ATTR([METRIC_DESC])
  }

measure: none_attribute_derived_2 {
    description: "Calculated field: ATTR([METRIC_RESULT_STR])"
    type: sum
    sql: ${none_attribute_derived_2_calc} ;;
