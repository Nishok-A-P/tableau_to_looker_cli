...
measure: issuecounttot {
  ...
  dimension: scheduledesc_attribute_derived {
    ...
    type: string
    sql: ANY_VALUE(${scheduledesc}) ;;
    # Original Tableau formula: ATTR([Calculation_1071012336442302472])
  }

  dimension: none_attribute_derived {
    description: "Calculated field: ATTR([Issue Desc])"
    type: string
    sql: ANY_VALUE(${TABLE}.`Issue Desc`) ;;
    # Original Tableau formula: ATTR([Issue Desc])
  }
...
measure: none_max_derived {
    description: "Calculated field: MAX([Tcdvol Tlktm Len])"
    type: number
    sql: MAX(${TABLE}.`Tcdvol Tlktm Len`) ;;


    # Original Tableau formula: MAX([Tcdvol Tlktm Len])
  }
