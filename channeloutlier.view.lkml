view: channeloutlier {
  ...
  sql_table_name: "public"."ChannelOutlier" ;;
  ...
  dimension: inb_aht_sec_threshold_calc {
    description: "Row-level calculation for inb_aht_sec_threshold: FLOAT([Parameters].[Box Close % Target (copy)_260082918621159424])"
    type: number
    sql: CAST({% parameter box_close_target_copy_260082918621159424 %} AS DOUBLE PRECISION) ;;
    hidden: yes
    # Original Tableau formula: FLOAT([Parameters].[Box Close % Target (copy)_260082918621159424])
  }

  dimension: inb_transfer_percent_threshold_calc {
    description: "Row-level calculation for inb_transfer_percent_threshold: FLOAT([Parameters].[Box Close % Target (copy)_260082918624153603])"
    type: number
    sql: CAST({% parameter box_close_target_copy_260082918624153603 %} AS DOUBLE PRECISION) ;;
    hidden: yes
    # Original Tableau formula: FLOAT([Parameters].[Box Close % Target (copy)_260082918624153603])
  }

  dimension: sales_time_percent_threshold_calc {
    description: "Row-level calculation for sales_time_percent_threshold: FLOAT([Parameters].[Box Close % Target (copy)_260082918625116166])"
    type: number
    sql: CAST({% parameter box_close_target_copy_260082918625116166 %} AS DOUBLE PRECISION) ;;
    hidden: yes
    # Original Tableau formula: FLOAT([Parameters].[Box Close % Target (copy)_260082918625116166])
  }

  dimension: inb_hold_percent_threshold_calc {
    description: "Row-level calculation for inb_hold_percent_threshold: FLOAT([Parameters].[Box Close % Target (copy)_260082918625714185])"
    type: number
    sql: CAST({% parameter box_close_target_copy_260082918625714185 %} AS DOUBLE PRECISION) ;;
    hidden: yes
    # Original Tableau formula: FLOAT([Parameters].[Box Close % Target (copy)_260082918625714185])
  }

  dimension: box_close_percent_threshold_calc {
    description: "Row-level calculation for box_close_percent_threshold: FLOAT([Parameters].[Parameter 7])"
    type: number
    sql: CAST({% parameter parameter_7 %} AS DOUBLE PRECISION) ;;
    hidden: yes
    # Original Tableau formula: FLOAT([Parameters].[Parameter 7])
  }

  dimension: inb_aht_sec_threshold_attribute_derived_calc {
    description: "Row-level calculation for inb_aht_sec_threshold_attribute_derived: ATTR([Box Close % Target (copy)_260082918621556738])"
    type: number
    sql: CASE WHEN MIN(${inb_aht_sec_threshold_calc}) IS NULL THEN NULL WHEN MIN(${inb_aht_sec_threshold_calc}) = MAX(${inb_aht_sec_threshold_calc}) THEN MIN(${inb_aht_sec_threshold_calc}) ELSE '*' END ;;
    hidden: yes
    # Original Tableau formula: ATTR([Box Close % Target (copy)_260082918621556738])
  }

  dimension: inb_transfer_percent_threshold_attribute_derived_calc {
    description: "Row-level calculation for inb_transfer_percent_threshold_attribute_derived: ATTR([Box Close % Target (copy)_260082918624559108])"
    type: number
    sql: CASE WHEN MIN(${inb_transfer_percent_threshold_calc}) IS NULL THEN NULL WHEN MIN(${inb_transfer_percent_threshold_calc}) = MAX(${inb_transfer_percent_threshold_calc}) THEN MIN(${inb_transfer_percent_threshold_calc}) ELSE '*' END ;;
    hidden: yes
    # Original Tableau formula: ATTR([Box Close % Target (copy)_260082918624559108])
  }

  dimension: sales_time_percent_threshold_attribute_derived_calc {
    description: "Row-level calculation for sales_time_percent_threshold_attribute_derived: ATTR([Box Close % Target (copy)_260082918625312775])"
    type: number
    sql: CASE WHEN MIN(${sales_time_percent_threshold_calc}) IS NULL THEN NULL WHEN MIN(${sales_time_percent_threshold_calc}) = MAX(${sales_time_percent_threshold_calc}) THEN MIN(${sales_time_percent_threshold_calc}) ELSE '*' END ;;
    hidden: yes
    # Original Tableau formula: ATTR([Box Close % Target (copy)_260082918625312775])
  }

  dimension: inb_hold_percent_threshold_attribute_derived_calc {
    description: "Row-level calculation for inb_hold_percent_threshold_attribute_derived: ATTR([Box Close % Target (copy)_260082918625886218])"
    type: number
    sql: CASE WHEN MIN(${inb_hold_percent_threshold_calc}) IS NULL THEN NULL WHEN MIN(${inb_hold_percent_threshold_calc}) = MAX(${inb_hold_percent_threshold_calc}) THEN MIN(${inb_hold_percent_threshold_calc}) ELSE '*' END ;;
    hidden: yes
    # Original Tableau formula: ATTR([Box Close % Target (copy)_260082918625886218])
  }

  dimension: box_close_percent_threshold_attribute_derived_calc {
    description: "Row-level calculation for box_close_percent_threshold_attribute_derived: ATTR([Calculation_171136824948584448])"
    type: number
    sql: CASE WHEN MIN(${box_close_percent_threshold_calc}) IS NULL THEN NULL WHEN MIN(${box_close_percent_threshold_calc}) = MAX(${box_close_percent_threshold_calc}) THEN MIN(${box_close_percent_threshold_calc}) ELSE '*' END ;;
    hidden: yes
    # Original Tableau formula: ATTR([Calculation_171136824948584448])
  }
...
measure: inb_aht_sec_performance {
    ...
    type: number
    sql: CASE WHEN ((${inb_aht_sec} < ${inb_aht_sec_threshold}) AND (${inb_aht_sec} > 0)) THEN 'Below Threshold' ELSE CASE WHEN (${inb_aht_sec} >= ${inb_aht_sec_threshold}) THEN 'Above Threshold' ELSE CASE WHEN (${inb_aht_sec} = 0) THEN 'Null' ELSE NULL END END END ;;
...
measure: inb_transfer_percent_performance {
    ...
    type: number
    sql: CASE WHEN (${inb_transfer_percent} < ${inb_transfer_percent_threshold}) THEN 'Below Threshold' ELSE CASE WHEN (${inb_transfer_percent} >= ${inb_transfer_percent_threshold}) THEN 'Above Threshold' ELSE NULL END END ;;
...
measure: sales_time_percent_performance {
    ...
    type: number
    sql: CASE WHEN (${sales_time_percent} < ${sales_time_percent_threshold}) THEN 'Below Threshold' ELSE CASE WHEN (${sales_time_percent} >= ${sales_time_percent_threshold}) THEN 'Above Threshold' ELSE NULL END END ;;
...
measure: inb_hold_percent_performance {
    ...
    type: number
    sql: CASE WHEN (${inb_hold_percent} < ${inb_hold_percent_threshold}) THEN 'Below Threshold' ELSE CASE WHEN (${inb_hold_percent} >= ${inb_hold_percent_threshold}) THEN 'Above Threshold' ELSE NULL END END ;;
...
measure: inb_transfer_percent {
    ...
    type: number
    sql: CASE WHEN (SUM(${TABLE}."Clstrnsout") <> 0) THEN (CAST(SUM(${TABLE}."Clstrnsout") AS DOUBLE PRECISION) / NULLIF(SUM(${TABLE}."Clsinbhndl"), 0)) ELSE 0 END ;;
...
measure: sales_time_percent {
    ...
    type: number
    sql: CASE WHEN (SUM(${TABLE}."Acdsignonm") <> 0) THEN (CAST(SUM(${TABLE}."Acdsignonm") AS DOUBLE PRECISION) / NULLIF(SUM(${TABLE}."Acdlstden0"), 0)) ELSE 0 END ;;
...
measure: sign_on_hours {
    ...
    type: number
    sql: CASE WHEN (${acd_sign_on_hours} < 20) THEN 'Under 20' ELSE CASE WHEN ((${acd_sign_on_hours} >= 20) AND (${acd_sign_on_hours} < 40)) THEN '20-40 Hours' ELSE CASE WHEN ((${acd_sign_on_hours} >= 40) AND (${acd_sign_on_hours} < 60)) THEN '40-60 Hours' ELSE CASE WHEN (${acd_sign_on_hours} >= 60) THEN '60+ Hours' ELSE NULL END END END END ;;
...
measure: calculation_171136824952332290 {
    ...
    # MIGRATION_ERROR: Could not convert calculated field ORIGINAL_FORMULA: If [federated.0o5uu5u1qiv8gf17rd80n0u85dbo].[Calculation_2493868328550068237] < [Calculation_171136824948584448] then 'Below Threshold' ElseIf [federated.0o5uu5u1qiv8gf17rd80n0u85dbo].[Calculation_2493868328550068237] >= [Calculation_171136824948584448] then 'Above Threshold' END CONVERSION_ERROR: No AST data available - formula parsing may have failed TODO: Manual migration required - please convert this formula manually
  }
...
measure: boxes_close_percent {
    ...
    type: number
    sql: CASE WHEN (SUM(${TABLE}."Boxeisup00") <> 0) THEN (CAST(SUM(${TABLE}."Boxeisup00") AS DOUBLE PRECISION) / NULLIF(SUM(${TABLE}."Clsinbhndl"), 0)) ELSE 0 END ;;
...
measure: inb_hold_percent {
    ...
    type: number
    sql: CASE WHEN (SUM(${TABLE}."Clsinbhldm") > 0) THEN (CAST(SUM(${TABLE}."Clsinbhldm") AS DOUBLE PRECISION) / NULLIF(SUM(${TABLE}."Acdiahtnum"), 0)) ELSE 0 END ;;
...
measure: inb_aht_sec {
    ...
    type: number
    sql: CASE WHEN (SUM(${TABLE}."Clsinbhndl") <> 0) THEN (CAST(SUM(${TABLE}."Ahtinbsnum") AS DOUBLE PRECISION) / NULLIF(SUM(${TABLE}."Clsinbhndl"), 0)) ELSE 0 END ;;
  ...
  dimension: rprt_date_my_derived {
    description: "Calculated field: STR(DATEPART('month', [Rprt_Date])) + '-' + STR(YEAR([Rprt_Date]))"
    type: string
    sql: ((CAST(EXTRACT(MONTH FROM ${TABLE}.`Rprt_Date`) AS STRING) || '-') || CAST(EXTRACT(YEAR FROM ${TABLE}.`Rprt_Date`) AS STRING)) ;;
    # Original Tableau formula: STR(DATEPART('month', [Rprt_Date])) + '-' + STR(YEAR([Rprt_Date]))
  }

measure: inb_aht_sec_threshold_attribute_derived {
    description: "Calculated field: ATTR([Box Close % Target (copy)_260082918621556738])"
    type: sum
    sql: ${inb_aht_sec_threshold_attribute_derived_calc} ;;


    # Original Tableau formula: ATTR([Box Close % Target (copy)_260082918621556738])
  }

measure: inb_transfer_percent_threshold_attribute_derived {
    description: "Calculated field: ATTR([Box Close % Target (copy)_260082918624559108])"
    type: sum
    sql: ${inb_transfer_percent_threshold_attribute_derived_calc} ;;


    # Original Tableau formula: ATTR([Box Close % Target (copy)_260082918624559108])
  }

measure: sales_time_percent_threshold_attribute_derived {
    description: "Calculated field: ATTR([Box Close % Target (copy)_260082918625312775])"
    type: sum
    sql: ${sales_time_percent_threshold_attribute_derived_calc} ;;


    # Original Tableau formula: ATTR([Box Close % Target (copy)_260082918625312775])
  }

measure: inb_hold_percent_threshold_attribute_derived {
    description: "Calculated field: ATTR([Box Close % Target (copy)_260082918625886218])"
    type: sum
    sql: ${inb_hold_percent_threshold_attribute_derived_calc} ;;


    # Original Tableau formula: ATTR([Box Close % Target (copy)_260082918625886218])
  }

measure: box_close_percent_threshold_attribute_derived {
    description: "Calculated field: ATTR([Calculation_171136824948584448])"
    type: sum
    sql: ${box_close_percent_threshold_attribute_derived_calc} ;;


    # Original Tableau formula: ATTR([Calculation_171136824948584448])
  }

measure: rprt_date_max_derived {
    description: "Calculated field: MAX([Rprt_Date])"
    type: number
    sql: MAX(${TABLE}."Rprt_Date") ;;


    # Original Tableau formula: MAX([Rprt_Date])
  }
