...
measure: rev_run {
    ...
    type: number
    sql: CASE WHEN ANY_VALUE(${is_current_month}) THEN ${revenue_run_rate} ELSE ${revenue} END ;;
...
measure: selected_month_sales_denominator_sum_derived {
  ...
  dimension: sls_outlet_nm_attribute_derived {
    ...
    type: string
    sql: ANY_VALUE(${TABLE}.`SLS_OUTLET_NM`) ;;
