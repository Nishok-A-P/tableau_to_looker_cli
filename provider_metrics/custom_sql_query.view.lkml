...
measure: p_current_year_var_yoy {
    ...
    type: number
    sql: (IFNULL((${p_mertics_list_yoy} - 'MIGRATION_REQUIRED'), 0) / NULLIF(ABS('MIGRATION_REQUIRED'), 0)) ;;
  ...
  dimension: p_username_filter {
    ...
    type: yesno
    sql: ((({% parameter parameter_7 %} = 'Switch to All Aspen View') AND (${TABLE}.`User Name` = 'ALL')) OR (({% parameter parameter_7 %} = 'Switch to My Office(s) View') AND (${TABLE}.`User Name` = '{{ _user_attributes['name']}}'))) ;;
