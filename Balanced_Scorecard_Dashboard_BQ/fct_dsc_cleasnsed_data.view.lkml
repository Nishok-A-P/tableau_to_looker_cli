view: fct_dsc_cleasnsed_data {
  ...
  dimension: none_countd_derived_calc {
    description: "Row-level calculation for none_countd_derived: COUNTD([Division VP])"
    type: number
  ...
  dimension: none_countd_derived_2_calc {
    description: "Row-level calculation for none_countd_derived_2: COUNTD([FacilityCode1])"
    type: number
  ...
  dimension: none_countd_derived_3_calc {
    description: "Row-level calculation for none_countd_derived_3: COUNTD([Region Description])"
    type: number
  ...
  dimension: none_countd_derived_4_calc {
    description: "Row-level calculation for none_countd_derived_4: COUNTD([Territory])"
    type: number
...
measure: sortfield_om {
    ...
    type: number
    sql: CASE WHEN ({% parameter parameter_2_1 %} = 'Alphabetical') THEN CAST((CAST(MIN(UPPER(${TABLE}.`Facility Code`)) AS STRING) || CAST(MIN(UPPER(SUBSTR(${TABLE}.`Facility Code`, 2, 1))) AS STRING)) AS FLOAT64) WHEN ({% parameter parameter_2_1 %} = 'Overall') THEN (SUM(${om_overall_score_num}) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Net Promoter Score') THEN (SUM(${TABLE}.`NetPromoterScore`) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Rolling 12-month TO %') THEN (SUM(${TABLE}.`Office TO`) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Credit App %') THEN (SUM(${TABLE}.`Credit App`) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'NP Completion %') THEN (SUM(${TABLE}.`Completion Percent`) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Gross Profit vs Budget') THEN (${gross_profit_rollup_om} * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'EBITDA vs Budget') THEN (SUM(${TABLE}.`EBITD Avs Bud`) * CAST({% parameter parameter_3_1 %} AS INT64)) END ;;
  ...
  dimension: territory_new_2 {
    ...
    type: yesno
    sql: ((/* Cannot convert complex LOD function - contains IF/CASE/nested conditions and Functions */ + /* Cannot convert complex LOD function - contains IF/CASE/nested conditions and Functions */) >= 1) ;;
...
measure: office_to_rollup {
  ...
  dimension: max_date {
    ...
    type: string
    sql: ( SELECT MAX(${first_of_month}) FROM `tableau-to-looker-migration.Super_Store_Sales.Balance Scorecard OT BSC` as fct_dsc_cleasnsed_data -- LOD
            ) ;;
...
measure: credit_app_rollup {
  ...
  dimension: usernamecalculated {
    ...
    type: string
    sql: CASE WHEN ({% parameter parameter_5 %} = 'MY OFFICES') THEN LOWER('MIGRATION_REQUIRED') ELSE 'A000000' END ;;
...
measure: usernameposition {
  ...
  dimension: in_hierarchy {
    ...
    type: yesno
    sql: (( SELECT SUM(${usernameposition}) FROM `tableau-to-looker-migration.Super_Store_Sales.Balance Scorecard OT BSC` as fct_dsc_cleasnsed_data -- LOD
            ) != 0) ;;
...
measure: om_np_tx_accepted_per_np_num_sum_derived {
    ...
    type: number
    sql: SUM(${om_np_tx_accepted_per_np_num}) ;;
...
measure: om_gross_profit_variance_to_budget_num_sum_derived {
    ...
    type: number
    sql: SUM(${om_gross_profit_variance_to_budget_num}) ;;
...
measure: none_countd_derived_3 {
    description: "Calculated field: COUNTD([Region Description])"
    type: countd
    sql: ${none_countd_derived_3_calc} ;;


    # Original Tableau formula: COUNTD([Region Description])
  }

measure: none_countd_derived_4 {
    description: "Calculated field: COUNTD([Territory])"
    type: countd
    sql: ${none_countd_derived_4_calc} ;;
