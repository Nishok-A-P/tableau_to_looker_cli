view: fct_dsc_cleasnsed_data {
  ...
  dimension: division_vp_countd_derived_calc {
    description: "Row-level calculation for division_vp_countd_derived: COUNTD([Division VP])"
    type: number
    sql: COUNT(DISTINCT ${TABLE}.`Division VP`) ;;
    hidden: yes
    # Original Tableau formula: COUNTD([Division VP])
  }

  dimension: facilitycode1_countd_derived_calc {
    description: "Row-level calculation for facilitycode1_countd_derived: COUNTD([FacilityCode1])"
    type: number
    sql: COUNT(DISTINCT ${TABLE}.`FacilityCode1`) ;;
    hidden: yes
    # Original Tableau formula: COUNTD([FacilityCode1])
  }

  dimension: region_description_countd_derived_calc {
    description: "Row-level calculation for region_description_countd_derived: COUNTD([Region Description])"
    type: number
    sql: COUNT(DISTINCT ${TABLE}.`Region Description`) ;;
    hidden: yes
    # Original Tableau formula: COUNTD([Region Description])
  }

  dimension: territory_countd_derived_calc {
    description: "Row-level calculation for territory_countd_derived: COUNTD([Territory])"
    type: number
    sql: COUNT(DISTINCT ${TABLE}.`Territory`) ;;
    hidden: yes
    # Original Tableau formula: COUNTD([Territory])
  }
...
measure: sortfield_om {
    ...
    type: number
    sql: CASE WHEN ({% parameter parameter_2_1 %} = 'Alphabetical') THEN CAST((CAST(MIN(UPPER(${TABLE}.`Facility Code`)) AS STRING) || CAST(MIN(UPPER(SUBSTR(${TABLE}.`Facility Code`, 2, 1))) AS STRING)) AS FLOAT64) WHEN ({% parameter parameter_2_1 %} = 'Overall') THEN (SUM(${om_overall_score_num_calc}) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Net Promoter Score') THEN (SUM(${TABLE}.`NetPromoterScore`) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Rolling 12-month TO %') THEN (SUM(${TABLE}.`Office TO`) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Credit App %') THEN (SUM(${TABLE}.`Credit App`) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'NP Completion %') THEN (SUM(${TABLE}.`Completion Percent`) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Gross Profit vs Budget') THEN (${gross_profit_rollup_om} * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'EBITDA vs Budget') THEN (SUM(${TABLE}.`EBITD Avs Bud`) * CAST({% parameter parameter_3_1 %} AS INT64)) END ;;
...
measure: om_overall_score_num {
  ...
  dimension: rm_scores_nps {
    ...
    type: string
    sql: (SELECT CASE WHEN (${net_promoter_score_rollup} >= ROUND(AVG(${TABLE}.`NPSSAT _Union_`), 3)) THEN 'SAT' ELSE CASE WHEN (${net_promoter_score_rollup} >= ROUND(AVG(${TABLE}.`NPSATLow _Union_`), 3)) THEN 'AT' ELSE CASE WHEN ((${net_promoter_score_rollup} >= ROUND(AVG(${TABLE}.`NPSOTLow _Union_`), 3)) OR ${net_promoter_score_rollup} IS NULL) THEN 'OT' ELSE CASE WHEN (${net_promoter_score_rollup} >= ROUND(AVG(${TABLE}.`NPSBTLow _Union_`), 3)) THEN 'BT' ELSE CASE WHEN (${net_promoter_score_rollup} < ROUND(AVG(${TABLE}.`NPSBTLow _Union_`), 3)) THEN 'SBT' ELSE NULL END END END END END FROM ${TABLE} GROUP BY `Region Description`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
...
measure: credit_app_rollup {
  ...
  dimension: usernamecalculated {
    ...
    type: string
    sql: CASE WHEN ({% parameter parameter_5 %} = 'MY OFFICES') THEN LOWER('{{ _user_attributes['name']}}') ELSE 'A000000' END ;;
...
measure: td_overall_score_num {
  ...
  dimension: rm_scores_completion {
    ...
    type: string
    sql: (SELECT CASE WHEN (${completion_rollup} >= ROUND(AVG(${TABLE}.`NPCompletionSAT _Union_`), 3)) THEN 'SAT' ELSE CASE WHEN (${completion_rollup} >= ROUND(AVG(${TABLE}.`NPCompletionATLow _Union_`), 3)) THEN 'AT' ELSE CASE WHEN ((${completion_rollup} >= ROUND(AVG(${TABLE}.`NPCompletionOTLow _Union_`), 3)) OR ${completion_rollup} IS NULL) THEN 'OT' ELSE CASE WHEN (${completion_rollup} >= ROUND(AVG(${TABLE}.`NPCompletionBTLow _Union_`), 3)) THEN 'BT' ELSE CASE WHEN (${completion_rollup} < ROUND(AVG(${TABLE}.`NPCompletionBTLow _Union_`), 3)) THEN 'SBT' ELSE NULL END END END END END FROM ${TABLE} GROUP BY `Region Description`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
    # Original Tableau formula: { FIXED [Region Description],[Calculation_2087136979212263426],[Calculation_2087136979212402691], [Calculation_2699345060414803975]: (IF [Calculation_705376323849363457] >= Round(AVG([NPCompletionSAT _Union_]),3) Then 'SAT' ELSEIF [Calculation_705376323849363457] >= Round(AVG([NPCompletionATLow _Union_]),3) Then 'AT' ELSEIF [Calculation_705376323849363457] >= Round(AVG([NPCompletionOTLow _Union_]),3) OR ISNULL([Calculation_705376323849363457]) Then 'OT' ELSEIF [Calculation_705376323849363457] >= Round(AVG([NPCompletionBTLow _Union_]),3) Then 'BT' ELSEIF [Calculation_705376323849363457] < Round(AVG([NPCompletionBTLow _Union_]),3) Then 'SBT' END)}
  }
  ...
  dimension: rm_scores_credit_apps {
    ...
    type: string
    sql: (SELECT CASE WHEN (${credit_app_rollup} >= ROUND(AVG(${TABLE}.`CreditAppsSAT _Union_`), 3)) THEN 'SAT' ELSE CASE WHEN (${credit_app_rollup} >= ROUND(AVG(${TABLE}.`CreditAppsATLow _Union_`), 3)) THEN 'AT' ELSE CASE WHEN ((${credit_app_rollup} >= ROUND(AVG(${TABLE}.`CreditAppsOTLow _Union_`), 3)) OR ${credit_app_rollup} IS NULL) THEN 'OT' ELSE CASE WHEN (${credit_app_rollup} >= ROUND(AVG(${TABLE}.`CreditAppsBTLow _Union_`), 3)) THEN 'BT' ELSE CASE WHEN (${credit_app_rollup} < ROUND(AVG(${TABLE}.`CreditAppsBTLow _Union_`), 3)) THEN 'SBT' ELSE NULL END END END END END FROM ${TABLE} GROUP BY `Region Description`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
    # Original Tableau formula: { FIXED [Region Description],[Calculation_2087136979212263426],[Calculation_2087136979212402691], [Calculation_2699345060414803975]: (IF [Calculation_705376323849728002] >= Round(AVG([CreditAppsSAT _Union_]),3) Then 'SAT' ELSEIF [Calculation_705376323849728002] >= Round(AVG([CreditAppsATLow _Union_]),3) Then 'AT' ELSEIF [Calculation_705376323849728002] >= Round(AVG([CreditAppsOTLow _Union_]),3) OR ISNULL([Calculation_705376323849728002]) Then 'OT' ELSEIF [Calculation_705376323849728002] >= Round(AVG([CreditAppsBTLow _Union_]),3) Then 'BT' ELSEIF [Calculation_705376323849728002] < Round(AVG([CreditAppsBTLow _Union_]),3) Then 'SBT' END)}
  }
  ...
  dimension: td_scores_completion {
    ...
    type: string
    sql: (SELECT CASE WHEN (${completion_rollup} >= ROUND(AVG(${TABLE}.`NPCompletionSAT _Union_`), 3)) THEN 'SAT' ELSE CASE WHEN ${completion_rollup} IS NULL THEN NULL ELSE CASE WHEN (${completion_rollup} >= ROUND(AVG(${TABLE}.`NPCompletionATLow _Union_`), 3)) THEN 'AT' ELSE CASE WHEN (${completion_rollup} >= ROUND(AVG(${TABLE}.`NPCompletionOTLow _Union_`), 3)) THEN 'OT' ELSE CASE WHEN (${completion_rollup} >= ROUND(AVG(${TABLE}.`NPCompletionBTLow _Union_`), 3)) THEN 'BT' ELSE CASE WHEN (${completion_rollup} < ROUND(AVG(${TABLE}.`NPCompletionBTLow _Union_`), 3)) THEN 'SBT' ELSE NULL END END END END END END FROM ${TABLE} GROUP BY `Territory`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
  ...
  dimension: rm_scores_officeto {
    ...
    type: string
    sql: (SELECT CASE WHEN ((EXTRACT(MONTH FROM CURRENT_DATE()) = EXTRACT(MONTH FROM {% parameter mydate_month_parameter %})) AND (EXTRACT(YEAR FROM CURRENT_DATE()) = EXTRACT(YEAR FROM {% parameter mydate_month_parameter %}))) THEN 'NULL' ELSE CASE WHEN (${office_to_rollup} < ROUND(AVG(${TABLE}.`TurnoverSAT _Union_`), 4)) THEN 'SAT' ELSE CASE WHEN (${office_to_rollup} <= ROUND(AVG(${TABLE}.`TurnoverATLow _Union_`), 4)) THEN 'AT' ELSE CASE WHEN ((${office_to_rollup} <= ROUND(AVG(${TABLE}.`TurnoverOTLow _Union_`), 4)) OR ${office_to_rollup} IS NULL) THEN 'OT' ELSE CASE WHEN (${office_to_rollup} <= ROUND(AVG(${TABLE}.`TurnoverBTLow _Union_`), 4)) THEN 'BT' ELSE CASE WHEN (${office_to_rollup} > ROUND(AVG(${TABLE}.`TurnoverBTLow _Union_`), 4)) THEN 'SBT' ELSE NULL END END END END END END FROM ${TABLE} GROUP BY `Region Description`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
    # Original Tableau formula: { FIXED [Region Description],[Calculation_2087136979212263426],[Calculation_2087136979212402691], [Calculation_2699345060414803975]: (IF Month(TODAY()) = Month([Parameters].[MyDate Month Parameter]) and Year(TODAY()) = Year([Parameters].[MyDate Month Parameter]) Then 'NULL' ELSEIF [Calculation_361976852014145543] < Round(AVG([TurnoverSAT _Union_]),4) Then 'SAT' ELSEIF [Calculation_361976852014145543] <= Round(AVG([TurnoverATLow _Union_]),4) Then 'AT' ELSEIF [Calculation_361976852014145543] <= Round(AVG([TurnoverOTLow _Union_]),4) OR ISNULL([Calculation_361976852014145543]) Then 'OT' ELSEIF [Calculation_361976852014145543] <= Round(AVG([TurnoverBTLow _Union_]),4) Then 'BT' ELSEIF [Calculation_361976852014145543] > Round(AVG([TurnoverBTLow _Union_]),4) Then 'SBT' END)}
  }
  ...
  dimension: rm_scores_gross_profit {
    ...
    type: string
    sql: (SELECT CASE WHEN ((EXTRACT(MONTH FROM CURRENT_DATE()) = EXTRACT(MONTH FROM {% parameter mydate_month_parameter %})) AND (EXTRACT(YEAR FROM CURRENT_DATE()) = EXTRACT(YEAR FROM {% parameter mydate_month_parameter %}))) THEN 'NULL' ELSE CASE WHEN (${gross_profit_rollup_rm} >= ROUND(AVG(${TABLE}.`GPvsBudgetSAT _Union_`), 4)) THEN 'SAT' ELSE CASE WHEN (${gross_profit_rollup_rm} >= ROUND(AVG(${TABLE}.`GPvsBudgetATLow _Union_`), 4)) THEN 'AT' ELSE CASE WHEN ((${gross_profit_rollup_rm} >= ROUND(AVG(${TABLE}.`GPvsBudgetOTLow _Union_`), 4)) OR ${gross_profit_rollup_rm} IS NULL) THEN 'OT' ELSE CASE WHEN (${gross_profit_rollup_rm} >= ROUND(AVG(${TABLE}.`GPvsBudgetBTLow _Union_`), 4)) THEN 'BT' ELSE CASE WHEN (${gross_profit_rollup_rm} < ROUND(AVG(${TABLE}.`GPvsBudgetBTLow _Union_`), 4)) THEN 'SBT' ELSE NULL END END END END END END FROM ${TABLE} GROUP BY `Region Description`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
    # Original Tableau formula: { FIXED [Region Description],[Calculation_2087136979212263426],[Calculation_2087136979212402691], [Calculation_2699345060414803975]: (IF Month(TODAY()) = Month([Parameters].[MyDate Month Parameter]) and Year(TODAY()) = Year([Parameters].[MyDate Month Parameter]) Then 'NULL' ElseIF [Gross Profit Rollup (copy)_547750316289679360] >= Round(AVG([GPvsBudgetSAT _Union_]),4) Then 'SAT' ELSEIF [Gross Profit Rollup (copy)_547750316289679360] >= Round(AVG([GPvsBudgetATLow _Union_]),4) Then 'AT' ELSEIF [Gross Profit Rollup (copy)_547750316289679360] >= Round(AVG([GPvsBudgetOTLow _Union_]),4) OR ISNULL([Gross Profit Rollup (copy)_547750316289679360]) Then 'OT' ELSEIF [Gross Profit Rollup (copy)_547750316289679360] >= Round(AVG([GPvsBudgetBTLow _Union_]),4) Then 'BT' ELSEIF [Gross Profit Rollup (copy)_547750316289679360] < Round(AVG([GPvsBudgetBTLow _Union_]),4) Then 'SBT' END)}
  }
  ...
  dimension: rm_scores_np_tx_accepted_per_np {
    ...
    type: string
    sql: (SELECT CASE WHEN (${np_tx_accepted_rollup} >= ROUND(AVG(${TABLE}.`NPTxAcceptedSAT _Union_`), 0)) THEN 'SAT' ELSE CASE WHEN (${np_tx_accepted_rollup} >= ROUND(AVG(${TABLE}.`NPTxAcceptedATLow _Union_`), 0)) THEN 'AT' ELSE CASE WHEN ((${np_tx_accepted_rollup} >= ROUND(AVG(${TABLE}.`NPTxAcceptedOTLow _Union_`), 0)) OR ${ebitda_rollup} IS NULL) THEN 'OT' ELSE CASE WHEN (${np_tx_accepted_rollup} >= ROUND(AVG(${TABLE}.`NPTxAcceptedBTLow _Union_`), 0)) THEN 'BT' ELSE CASE WHEN (${np_tx_accepted_rollup} < ROUND(AVG(${TABLE}.`NPTxAcceptedBTLow _Union_`), 0)) THEN 'SBT' ELSE NULL END END END END END FROM ${TABLE} GROUP BY `Region Description`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
    # Original Tableau formula: { FIXED [Region Description],[Calculation_2087136979212263426],[Calculation_2087136979212402691], [Calculation_2699345060414803975]: (IF [Net Promoter All Rollup (copy)_342555104449536003] >= Round(AVG([NPTxAcceptedSAT _Union_]),0) Then 'SAT' ELSEIF [Net Promoter All Rollup (copy)_342555104449536003] >= Round(AVG([NPTxAcceptedATLow _Union_]),0) Then 'AT' ELSEIF [Net Promoter All Rollup (copy)_342555104449536003] >= Round(AVG([NPTxAcceptedOTLow _Union_]),0) OR ISNULL([Calculation_2699345059241832452]) Then 'OT' ELSEIF [Net Promoter All Rollup (copy)_342555104449536003] >= Round(AVG([NPTxAcceptedBTLow _Union_]),0) Then 'BT' ELSEIF [Net Promoter All Rollup (copy)_342555104449536003] < Round(AVG([NPTxAcceptedBTLow _Union_]),0) Then 'SBT' END)}
  }
  ...
  dimension: td_scores_ebitda {
    ...
    type: string
    sql: (SELECT CASE WHEN ((EXTRACT(MONTH FROM CURRENT_DATE()) = EXTRACT(MONTH FROM {% parameter mydate_month_parameter %})) AND (EXTRACT(YEAR FROM CURRENT_DATE()) = EXTRACT(YEAR FROM {% parameter mydate_month_parameter %}))) THEN 'NULL' ELSE CASE WHEN (${ebitda_rollup} >= ROUND(AVG(${TABLE}.`EBITDAvsBudSAT _Union_`), 4)) THEN 'SAT' ELSE CASE WHEN (${ebitda_rollup} >= ROUND(AVG(${TABLE}.`EBITDAvsBudATLow _Union_`), 4)) THEN 'AT' ELSE CASE WHEN ((${ebitda_rollup} >= ROUND(AVG(${TABLE}.`EBITDAvsBudOTLow _Union_`), 4)) OR ${ebitda_rollup} IS NULL) THEN 'OT' ELSE CASE WHEN (${ebitda_rollup} >= ROUND(AVG(${TABLE}.`EBITDAvsBudBTLow _Union_`), 4)) THEN 'BT' ELSE CASE WHEN (${ebitda_rollup} < ROUND(AVG(${TABLE}.`EBITDAvsBudBTLow _Union_`), 4)) THEN 'SBT' ELSE NULL END END END END END END FROM ${TABLE} GROUP BY `Territory`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
    # Original Tableau formula: { FIXED [Territory],[Calculation_2087136979212263426],[Calculation_2087136979212402691], [Calculation_2699345060414803975]: (IF Month(TODAY()) = Month([Parameters].[MyDate Month Parameter]) and Year(TODAY()) = Year([Parameters].[MyDate Month Parameter]) Then 'NULL' ELSEIF [Calculation_2699345059241832452] >= Round(AVG([EBITDAvsBudSAT _Union_]),4) Then 'SAT' ELSEIF [Calculation_2699345059241832452] >= Round(AVG([EBITDAvsBudATLow _Union_]),4) Then 'AT' ELSEIF [Calculation_2699345059241832452] >= Round(AVG([EBITDAvsBudOTLow _Union_]),4) OR ISNULL([Calculation_2699345059241832452]) Then 'OT' ELSEIF [Calculation_2699345059241832452] >= Round(AVG([EBITDAvsBudBTLow _Union_]),4) Then 'BT' ELSEIF [Calculation_2699345059241832452] < Round(AVG([EBITDAvsBudBTLow _Union_]),4) Then 'SBT' END)}
  }
...
measure: sortfield_dvp {
    ...
    type: number
    sql: CASE WHEN ({% parameter parameter_2_1 %} = 'Alphabetical') THEN CAST((CAST(MIN(UPPER(${TABLE}.`Division VP`)) AS STRING) || CAST(MIN(UPPER(SUBSTR(${TABLE}.`Division VP`, 2, 1))) AS STRING)) AS FLOAT64) WHEN ({% parameter parameter_2_1 %} = 'Overall') THEN (AVG(${dvp_overall_score_num}) * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Net Promoter Score') THEN (${net_promoter_score_rollup} * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Office TO') THEN (${office_to_rollup} * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'NP Tx Accepted Per NP') THEN (${np_tx_accepted_rollup} * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Credit App %') THEN (${credit_app_rollup} * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'NP Completion %') THEN (${completion_rollup} * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'EBITDA vs Budget') THEN (${ebitda_rollup} * CAST({% parameter parameter_3_1 %} AS INT64)) WHEN ({% parameter parameter_2_1 %} = 'Gross Profit vs Budget') THEN (${gross_profit_rollup} * CAST({% parameter parameter_3_1 %} AS INT64)) END ;;
...
measure: dvp_overall_score_num {
  ...
  dimension: td_scores_np_tx_accepted {
    ...
    type: string
    sql: (SELECT CASE WHEN (${np_tx_accepted_rollup} >= ROUND(AVG(${TABLE}.`NPTxAcceptedSAT _Union_`), 2)) THEN 'SAT' ELSE CASE WHEN (${np_tx_accepted_rollup} >= ROUND(AVG(${TABLE}.`NPTxAcceptedATLow _Union_`), 2)) THEN 'AT' ELSE CASE WHEN (${np_tx_accepted_rollup} >= ROUND(AVG(${TABLE}.`NPTxAcceptedOTLow _Union_`), 2)) THEN 'OT' ELSE CASE WHEN (${np_tx_accepted_rollup} >= ROUND(AVG(${TABLE}.`NPTxAcceptedBTLow _Union_`), 2)) THEN 'BT' ELSE CASE WHEN (${np_tx_accepted_rollup} < ROUND(AVG(${TABLE}.`NPTxAcceptedBTLow _Union_`), 2)) THEN 'SBT' ELSE NULL END END END END END FROM ${TABLE} GROUP BY `Territory`, ${timing_parameter}, ${month_year}, ${unopened_office_filter}) ;;
    # Original Tableau formula: { FIXED [Territory],[Calculation_2087136979212263426],[Calculation_2087136979212402691], [Calculation_2699345060414803975]: (IF [Net Promoter All Rollup (copy)_342555104449536003] >= Round(AVG([NPTxAcceptedSAT _Union_]),2) Then 'SAT' ELSEIF [Net Promoter All Rollup (copy)_342555104449536003] >= Round(AVG([NPTxAcceptedATLow _Union_]),2) Then 'AT' ELSEIF [Net Promoter All Rollup (copy)_342555104449536003] >= Round(Avg([NPTxAcceptedOTLow _Union_]),2) Then 'OT' ELSEIF [Net Promoter All Rollup (copy)_342555104449536003] >= Round(Avg([NPTxAcceptedBTLow _Union_]),2) Then 'BT' ELSEIF [Net Promoter All Rollup (copy)_342555104449536003] < Round(Avg([NPTxAcceptedBTLow _Union_]),2) Then 'SBT' END)}
  }
  ...
  dimension: first_of_month_month_trunc_derived {
    description: "Calculated field: DATETRUNC('month', [Calculation_361976851997593603])"
    type: date
    sql: DATE_TRUNC(${first_of_month}, month) ;;
    # Original Tableau formula: DATETRUNC('month', [Calculation_361976851997593603])
  }

  dimension: metric_selector_map_metric_user_derived {
    description: "Calculated field: [Metric Selector Map (copy)_2699345059239723008]"
    type: string
    sql: ${metric_selector_map_metric} ;;
    # Original Tableau formula: [Metric Selector Map (copy)_2699345059239723008]
  }

measure: division_vp_countd_derived {
    description: "Calculated field: COUNTD([Division VP])"
    type: countd
    sql: ${division_vp_countd_derived_calc} ;;


    # Original Tableau formula: COUNTD([Division VP])
  }

measure: dvp_overall_score_num_avg_derived {
    description: "Calculated field: AVG([TD Overall Score Num (copy)_2087136981061210236])"
    type: number
    sql: AVG(${dvp_overall_score_num}) ;;


    # Original Tableau formula: AVG([TD Overall Score Num (copy)_2087136981061210236])
  }

measure: dvp_completion_numeric_sum_derived {
    description: "Calculated field: SUM([DVP - NPS Numeric (copy)_342555104460296196])"
    type: number
    sql: SUM(${dvp_completion_numeric}) ;;


    # Original Tableau formula: SUM([DVP - NPS Numeric (copy)_342555104460296196])
  }

measure: dvp_np_tx_accepted_numeric_sum_derived {
    description: "Calculated field: SUM([TD - HygFTE Numeric (copy)_2087136981061210238])"
    type: number
    sql: SUM(${dvp_np_tx_accepted_numeric}) ;;


    # Original Tableau formula: SUM([TD - HygFTE Numeric (copy)_2087136981061210238])
  }

measure: dvp_nps_numeric_sum_derived {
    description: "Calculated field: SUM([TD - NPS Numeric (copy)_2087136981061210232])"
    type: number
    sql: SUM(${dvp_nps_numeric}) ;;


    # Original Tableau formula: SUM([TD - NPS Numeric (copy)_2087136981061210232])
  }

measure: dvp_credit_apps_numeric_sum_derived {
    description: "Calculated field: SUM([TD - Office Hours Numeric (copy)_2087136981061210237])"
    type: number
    sql: SUM(${dvp_credit_apps_numeric}) ;;


    # Original Tableau formula: SUM([TD - Office Hours Numeric (copy)_2087136981061210237])
  }

measure: dvp_officeto_numeric_sum_derived {
    description: "Calculated field: SUM([TD - OfficeTO Numeric (copy)_2087136981061210233])"
    type: number
    sql: SUM(${dvp_officeto_numeric}) ;;


    # Original Tableau formula: SUM([TD - OfficeTO Numeric (copy)_2087136981061210233])
  }

measure: dvp_ebitda_numeric_sum_derived {
    description: "Calculated field: SUM([TD - Yes Today Numeric (copy)_2087136981061210235])"
    type: number
    sql: SUM(${dvp_ebitda_numeric}) ;;


    # Original Tableau formula: SUM([TD - Yes Today Numeric (copy)_2087136981061210235])
  }

measure: dvp_overall_score_num_sum_derived {
    description: "Calculated field: SUM([TD Overall Score Num (copy)_2087136981061210236])"
    type: number
    sql: SUM(${dvp_overall_score_num}) ;;


    # Original Tableau formula: SUM([TD Overall Score Num (copy)_2087136981061210236])
  }

measure: om_completion_numeric_sum_derived {
    description: "Calculated field: SUM([Avg Rev and Dep Numeric (copy)_2087136979570114570])"
    type: number
    sql: SUM(${om_completion_numeric_calc}) ;;


    # Original Tableau formula: SUM([Avg Rev and Dep Numeric (copy)_2087136979570114570])
  }

measure: om_np_tx_accepted_per_np_num_sum_derived {
    description: "Calculated field: SUM([Calculation_1472114181295464448])"
    type: number
    sql: SUM(${om_np_tx_accepted_per_np_num_calc}) ;;


    # Original Tableau formula: SUM([Calculation_1472114181295464448])
  }

measure: om_gross_profit_variance_to_budget_num_sum_derived {
    description: "Calculated field: SUM([Calculation_1472114181295722497])"
    type: number
    sql: SUM(${om_gross_profit_variance_to_budget_num_calc}) ;;


    # Original Tableau formula: SUM([Calculation_1472114181295722497])
  }

measure: om_overall_score_num_sum_derived {
    description: "Calculated field: SUM([Calculation_2087136979572551699])"
    type: number
    sql: SUM(${om_overall_score_num_calc}) ;;


    # Original Tableau formula: SUM([Calculation_2087136979572551699])
  }

measure: om_credit_app_numeric_sum_derived {
    description: "Calculated field: SUM([Completion Numeric (copy)_2087136979570245643])"
    type: number
    sql: SUM(${om_credit_app_numeric_calc}) ;;


    # Original Tableau formula: SUM([Completion Numeric (copy)_2087136979570245643])
  }

measure: completion_percent_sum_derived {
    description: "Calculated field: SUM([Completion Percent])"
    type: number
    sql: SUM(${TABLE}.`Completion Percent`) ;;


    # Original Tableau formula: SUM([Completion Percent])
  }

measure: credit_app_sum_derived {
    description: "Calculated field: SUM([Credit App])"
    type: number
    sql: SUM(${TABLE}.`Credit App`) ;;


    # Original Tableau formula: SUM([Credit App])
  }

measure: np_tx_accepted_per_np_sum_derived {
    description: "Calculated field: SUM([NP Tx Accepted Per NP])"
    type: number
    sql: SUM(${TABLE}.`NP Tx Accepted Per NP`) ;;


    # Original Tableau formula: SUM([NP Tx Accepted Per NP])
  }

measure: net_promoter_score_sum_derived {
    description: "Calculated field: SUM([NetPromoterScore])"
    type: number
    sql: SUM(${TABLE}.`NetPromoterScore`) ;;


    # Original Tableau formula: SUM([NetPromoterScore])
  }

measure: om_nps_numeric_sum_derived {
    description: "Calculated field: SUM([OM - Oral Health Numeric (copy)_2087136979572797460])"
    type: number
    sql: SUM(${om_nps_numeric_calc}) ;;


    # Original Tableau formula: SUM([OM - Oral Health Numeric (copy)_2087136979572797460])
  }

measure: om_officeto_numeric_sum_derived {
    description: "Calculated field: SUM([OM - Showrate Numeric (copy)_2087136979571781647])"
    type: number
    sql: SUM(${om_officeto_numeric_calc}) ;;


    # Original Tableau formula: SUM([OM - Showrate Numeric (copy)_2087136979571781647])
  }

measure: om_ranks_sum_derived {
    description: "Calculated field: SUM([OM Ranks])"
    type: number
    sql: SUM(${TABLE}.`OM Ranks`) ;;


    # Original Tableau formula: SUM([OM Ranks])
  }

measure: office_to_sum_derived {
    description: "Calculated field: SUM([Office TO])"
    type: number
    sql: SUM(${TABLE}.`Office TO`) ;;


    # Original Tableau formula: SUM([Office TO])
  }

measure: rm_overall_score_num_avg_derived {
    description: "Calculated field: AVG([OM Overall Score Num (copy)_2087136980012261420])"
    type: number
    sql: AVG(${rm_overall_score_num}) ;;


    # Original Tableau formula: AVG([OM Overall Score Num (copy)_2087136980012261420])
  }

measure: rm_nps_numeric_sum_derived {
    description: "Calculated field: SUM([OM - NPS Numeric (copy)_2087136980012261424])"
    type: number
    sql: SUM(${rm_nps_numeric}) ;;


    # Original Tableau formula: SUM([OM - NPS Numeric (copy)_2087136980012261424])
  }

measure: rm_officeto_numeric_sum_derived {
    description: "Calculated field: SUM([OM - OfficeTO Numeric (copy)_2087136980012261426])"
    type: number
    sql: SUM(${rm_officeto_numeric}) ;;


    # Original Tableau formula: SUM([OM - OfficeTO Numeric (copy)_2087136980012261426])
  }

measure: rm_gross_profit_numeric_sum_derived {
    description: "Calculated field: SUM([OM - Wage Rate Numeric (copy)_2087136980012261422])"
    type: number
    sql: SUM(${rm_gross_profit_numeric}) ;;


    # Original Tableau formula: SUM([OM - Wage Rate Numeric (copy)_2087136980012261422])
  }

measure: rm_np_tx_accepted_per_np_numeric_sum_derived {
    description: "Calculated field: SUM([OM - Yes Today Numeric (copy)_2087136980012261423])"
    type: number
    sql: SUM(${rm_np_tx_accepted_per_np_numeric}) ;;


    # Original Tableau formula: SUM([OM - Yes Today Numeric (copy)_2087136980012261423])
  }

measure: rm_completion_numeric_sum_derived {
    description: "Calculated field: SUM([OM - Completion Numeric (copy)_2087136980012261418])"
    type: number
    sql: SUM(${rm_completion_numeric}) ;;


    # Original Tableau formula: SUM([OM - Completion Numeric (copy)_2087136980012261418])
  }

measure: rm_credit_app_numeric_sum_derived {
    description: "Calculated field: SUM([OM - Credit App Numeric (copy)_2087136980012261421])"
    type: number
    sql: SUM(${rm_credit_app_numeric}) ;;


    # Original Tableau formula: SUM([OM - Credit App Numeric (copy)_2087136980012261421])
  }

measure: rm_overall_score_num_sum_derived {
    description: "Calculated field: SUM([OM Overall Score Num (copy)_2087136980012261420])"
    type: number
    sql: SUM(${rm_overall_score_num}) ;;


    # Original Tableau formula: SUM([OM Overall Score Num (copy)_2087136980012261420])
  }

measure: td_overall_score_num_avg_derived {
    description: "Calculated field: AVG([RM Overall Score Num (copy)_2087136981005688901])"
    type: number
    sql: AVG(${td_overall_score_num}) ;;


    # Original Tableau formula: AVG([RM Overall Score Num (copy)_2087136981005688901])
  }

measure: td_nps_numeric_sum_derived {
    description: "Calculated field: SUM([RM - NPS Numeric (copy)_2087136981005688892])"
    type: number
    sql: SUM(${td_nps_numeric}) ;;


    # Original Tableau formula: SUM([RM - NPS Numeric (copy)_2087136981005688892])
  }

measure: td_officeto_numeric_sum_derived {
    description: "Calculated field: SUM([RM - OfficeTO Numeric (copy)_2087136981005688893])"
    type: number
    sql: SUM(${td_officeto_numeric}) ;;


    # Original Tableau formula: SUM([RM - OfficeTO Numeric (copy)_2087136981005688893])
  }

measure: td_ebitda_numeric_sum_derived {
    description: "Calculated field: SUM([RM - Yes Today Numeric (copy)_2087136981005688897])"
    type: number
    sql: SUM(${td_ebitda_numeric}) ;;


    # Original Tableau formula: SUM([RM - Yes Today Numeric (copy)_2087136981005688897])
  }

measure: td_overall_score_num_sum_derived {
    description: "Calculated field: SUM([RM Overall Score Num (copy)_2087136981005688901])"
    type: number
    sql: SUM(${td_overall_score_num}) ;;


    # Original Tableau formula: SUM([RM Overall Score Num (copy)_2087136981005688901])
  }

measure: td_credit_apps_numeric_sum_derived {
    description: "Calculated field: SUM([TD - OfficeTO Numeric (copy)_2087136981043130483])"
    type: number
    sql: SUM(${td_credit_apps_numeric}) ;;


    # Original Tableau formula: SUM([TD - OfficeTO Numeric (copy)_2087136981043130483])
  }

measure: td_completion_numeric_sum_derived {
    description: "Calculated field: SUM([TD - OfficeTO Numeric (copy)_2087136981043945591])"
    type: number
    sql: SUM(${td_completion_numeric}) ;;


    # Original Tableau formula: SUM([TD - OfficeTO Numeric (copy)_2087136981043945591])
  }

measure: td_np_tx_accepted_numeric_sum_derived {
    description: "Calculated field: SUM([TD - Showrate Numeric (copy)_2087136981043130484])"
    type: number
    sql: SUM(${td_np_tx_accepted_numeric}) ;;


    # Original Tableau formula: SUM([TD - Showrate Numeric (copy)_2087136981043130484])
  }
...
measure: facilitycode1_countd_derived {
    description: "Calculated field: COUNTD([FacilityCode1])"
    type: countd
    sql: ${facilitycode1_countd_derived_calc} ;;


    # Original Tableau formula: COUNTD([FacilityCode1])
  }

  dimension: om_ranks_none_derived {
    description: "Calculated field: [OM Ranks]"
    type: number
    sql: ${TABLE}.`OM Ranks` ;;
    # Original Tableau formula: [OM Ranks]
  }

measure: region_description_countd_derived {
    description: "Calculated field: COUNTD([Region Description])"
    type: countd
    sql: ${region_description_countd_derived_calc} ;;


    # Original Tableau formula: COUNTD([Region Description])
  }

measure: territory_countd_derived {
    description: "Calculated field: COUNTD([Territory])"
    type: countd
    sql: ${territory_countd_derived_calc} ;;
