view: dim_countries {
  ...
  dimension: insert_dt_4_hour_formatted {
    type: string
    sql: FORMAT_TIMESTAMP('%l %p',
      TIMESTAMP_TRUNC(CAST(${insert_dt_4_raw} AS TIMESTAMP), HOUR)) ;;
    label: "Hour"
    order_by_field: insert_dt_4_raw
  }



  dimension_group: last_update_dt_4 {
    type: time
    timeframes: [raw, time, date, week, month, quarter, year]
  ...
  dimension: last_update_dt_4_hour_formatted {
    type: string
    sql: FORMAT_TIMESTAMP('%l %p',
      TIMESTAMP_TRUNC(CAST(${last_update_dt_4_raw} AS TIMESTAMP), HOUR)) ;;
    label: "Hour"
    order_by_field: last_update_dt_4_raw
  }



  dimension_group: dw_insert_dt_4 {
    type: time
    timeframes: [raw, time, date, week, month, quarter, year]
  ...
  dimension: dw_insert_dt_4_hour_formatted {
    type: string
    sql: FORMAT_TIMESTAMP('%l %p',
      TIMESTAMP_TRUNC(CAST(${dw_insert_dt_4_raw} AS TIMESTAMP), HOUR)) ;;
    label: "Hour"
    order_by_field: dw_insert_dt_4_raw
  }
