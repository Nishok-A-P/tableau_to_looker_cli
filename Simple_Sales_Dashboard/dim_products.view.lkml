view: dim_products {
  ...
  dimension: insert_dt_1_hour_formatted {
    type: string
    sql: FORMAT_TIMESTAMP('%l %p',
      TIMESTAMP_TRUNC(CAST(${insert_dt_1_raw} AS TIMESTAMP), HOUR)) ;;
    label: "Hour"
    order_by_field: insert_dt_1_raw
  }



  dimension_group: last_update_dt_1 {
    type: time
    timeframes: [raw, time, date, week, month, quarter, year]
  ...
  dimension: last_update_dt_1_hour_formatted {
    type: string
    sql: FORMAT_TIMESTAMP('%l %p',
      TIMESTAMP_TRUNC(CAST(${last_update_dt_1_raw} AS TIMESTAMP), HOUR)) ;;
    label: "Hour"
    order_by_field: last_update_dt_1_raw
  }
