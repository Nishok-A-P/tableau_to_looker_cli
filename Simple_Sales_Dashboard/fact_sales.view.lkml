...
measure: cust_key_count_derived {
    description: "Calculated field: COUNT([CUST_KEY])"
    type: number
    sql: COUNT(${TABLE}.`CUST_KEY`) ;;


    # Original Tableau formula: COUNT([CUST_KEY])
  }

measure: sales_sum_derived {
    description: "Calculated field: SUM([AMOUNT_SOLD])"
    type: number
    sql: SUM(${TABLE}.`AMOUNT_SOLD`) ;;


    # Original Tableau formula: SUM([AMOUNT_SOLD])
  }

measure: quantity_sold_sum_derived {
    description: "Calculated field: SUM([QUANTITY_SOLD])"
    type: number
    sql: SUM(${TABLE}.`QUANTITY_SOLD`) ;;


    # Original Tableau formula: SUM([QUANTITY_SOLD])
  }
